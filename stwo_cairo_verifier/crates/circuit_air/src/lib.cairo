...
pub fn verify_circuit(proof: CircuitProof, config: @CircuitVerifierConfig) {
    ...
    // The rust circuit prover sets `pcs_config.lifting_log_size = Some(...)`, mixed into
    // the channel via `PcsConfig::mix_into` in stwo. Cairo's `PcsConfig` doesn't carry
    // the field, so we pass it through `mix_into`'s parameter from `CircuitVerifierConfig`.
    ...
    verify(
        ...
        ref channel,
        SECURITY_BITS,
        Option::Some(*config.lifting_log_size),
    );
}
