            ...
            + OpcodeWitness,
    {
        ...
        scope(|s| {
            if let Some(gen) = self.add_opcode {
                s.spawn(|_| {
                    add_opcode_result = Some(B::write_add_opcode_trace(
                        gen,
            ...
            if let Some(gen) = self.assert_eq_opcode {
                s.spawn(|_| {
                    assert_eq_opcode_result = Some(B::write_assert_eq_trace(
                        gen,
                        opcode_mem_tables.as_ref().unwrap(),
                        self.memory_address_to_id.as_ref().unwrap(),
                        self.memory_id_to_big.as_ref().unwrap(),
            ...
            if let Some(gen) = self.assert_eq_opcode_imm {
                s.spawn(|_| {
                    assert_eq_opcode_imm_result = Some(B::write_assert_eq_imm_trace(
                        gen,
                        opcode_mem_tables.as_ref().unwrap(),
                        self.memory_address_to_id.as_ref().unwrap(),
                        self.memory_id_to_big.as_ref().unwrap(),
            ...
            if let Some(gen) = self.jnz_opcode_taken {
                s.spawn(|_| {
                    jnz_opcode_taken_result = Some(B::write_jnz_taken_trace(
                        gen,
                        opcode_mem_tables.as_ref().unwrap(),
                        self.memory_address_to_id.as_ref().unwrap(),
                        self.memory_id_to_big.as_ref().unwrap(),
    ...
    pub fn write_interaction_trace(
        ...
        scope(|s| {
            if let Some(gen) = self.add_opcode {
                s.spawn(|_| {
                    add_opcode_result =
                        Some(B::write_add_opcode_interaction(gen, common_lookup_elements));
                });
            }
            if let Some(gen) = self.add_opcode_small {
                s.spawn(|_| {
            ...
            if let Some(gen) = self.assert_eq_opcode {
                s.spawn(|_| {
                    assert_eq_opcode_result =
                        Some(B::write_assert_eq_interaction(gen, common_lookup_elements));
                });
            }
            if let Some(gen) = self.assert_eq_opcode_imm {
                s.spawn(|_| {
                    assert_eq_opcode_imm_result =
                        Some(B::write_assert_eq_imm_interaction(gen, common_lookup_elements));
                });
            }
            if let Some(gen) = self.assert_eq_opcode_double_deref {
                s.spawn(|_| {
            ...
            if let Some(gen) = self.jnz_opcode_taken {
                s.spawn(|_| {
                    jnz_opcode_taken_result =
                        Some(B::write_jnz_taken_interaction(gen, common_lookup_elements));
                });
            }
            if let Some(gen) = self.jump_opcode_abs {
                s.spawn(|_| {
        ...
        let add_opcode_interaction_claim = add_opcode_result.map(|(trace, claimed_sum)| {
            evals.extend(trace);
            AddInteractionClaim { claimed_sum }
        });
        ...
        let assert_eq_opcode_interaction_claim =
            assert_eq_opcode_result.map(|(trace, claimed_sum)| {
                evals.extend(trace);
                AssertEqInteractionClaim { claimed_sum }
            });
        ...
        let jnz_opcode_taken_interaction_claim =
            jnz_opcode_taken_result.map(|(trace, claimed_sum)| {
                evals.extend(trace);
                JnzTakenInteractionClaim { claimed_sum }
            });
