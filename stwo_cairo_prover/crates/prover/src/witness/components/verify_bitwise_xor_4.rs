...
impl ClaimGenerator {
    /// Dense `(a << 4) | b -> row` LUT (length `1 << 8`), the inversion of this
    ...
    pub fn input_to_row_lut(&self) -> Vec<u32> {
        ...
        assert_eq!(
            ...
            LUT_SIZE,
            "bitwise_xor_4 input_to_row map does not cover all 4-bit pairs"
        );
        let mut lut = vec![0u32; LUT_SIZE];
        for ([a, b, _xor], &row) in &self.input_to_row {
            lut[((a.0 as usize) << 4) | b.0 as usize] = row as u32;
        }
        lut
    }
