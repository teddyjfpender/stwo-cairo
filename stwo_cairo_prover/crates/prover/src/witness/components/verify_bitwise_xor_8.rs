...
impl ClaimGenerator {
    /// Dense `(a << 8) | b -> row` LUT (length `1 << 16`), the inversion of this
    /// generator's `input_to_row` map (the preprocessed-table layout, never a
    /// closed form). Consumed by the device count kernel.
    pub fn input_to_row_lut(&self) -> Vec<u32> {
        const LUT_SIZE: usize = 1 << 16;
        assert_eq!(
            self.input_to_row.len(),
            LUT_SIZE,
            "bitwise_xor_8 input_to_row map does not cover all 8-bit pairs"
        );
        let mut lut = vec![0u32; LUT_SIZE];
        for ([a, b, _xor], &row) in &self.input_to_row {
            lut[((a.0 as usize) << 8) | b.0 as usize] = row as u32;
        }
        lut
    }

    /// Merges relation-indexed device count tables (layout
    /// `counts[relation_index * (1 << LOG_SIZE) + row]`) into the multiplicity
    /// columns. Order-independent adds — byte-equal to the same `increase_at` calls.
