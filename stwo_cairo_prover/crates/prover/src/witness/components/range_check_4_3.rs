...
impl ClaimGenerator {
    ...
    /// Dense `(v0 << 3) | v1 -> row` LUT (slot bits [4, 3]) for the device feed.
    pub fn input_to_row_lut(&self) -> Vec<u32> {
        crate::witness::utils::dense_input_to_row_lut(&self.input_to_row, [4, 3])
    }
...
fn write_trace_simd(
    ...
    let range_check_4_3_column_0 = preprocessed_trace.get_column(&PreProcessedColumnId {
        id: "range_check_4_3_column_0".to_owned(),
    });
    let range_check_4_3_column_1 = preprocessed_trace.get_column(&PreProcessedColumnId {
        id: "range_check_4_3_column_1".to_owned(),
    });
