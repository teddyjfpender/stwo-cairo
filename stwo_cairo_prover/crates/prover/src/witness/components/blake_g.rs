// This file was created by the AIR team.

#![allow(unused_parens)]
use cairo_air::components::blake_g::{Claim, InteractionClaim, N_TRACE_COLUMNS};
use stwo::core::fields::qm31::SecureField;
use stwo_constraint_framework::{RawLogupTrace, RawLogupTraceGenerator};

use crate::witness::components::{
    verify_bitwise_xor_12, verify_bitwise_xor_4, verify_bitwise_xor_7, verify_bitwise_xor_8,
    verify_bitwise_xor_9,
};
use crate::witness::prelude::*;

pub type InputType = [UInt32; 6];
pub type PackedInputType = [PackedUInt32; 6];

#[derive(Default)]
pub struct ClaimGenerator {
    pub packed_inputs: Mutex<Vec<PackedInputType>>,
    pub remainder_inputs: Mutex<Vec<InputType>>,
}

impl ClaimGenerator {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn write_trace(
        self,
        verify_bitwise_xor_8_state: &verify_bitwise_xor_8::ClaimGenerator,
        verify_bitwise_xor_12_state: &verify_bitwise_xor_12::ClaimGenerator,
        verify_bitwise_xor_4_state: &verify_bitwise_xor_4::ClaimGenerator,
        verify_bitwise_xor_7_state: &verify_bitwise_xor_7::ClaimGenerator,
        verify_bitwise_xor_9_state: &verify_bitwise_xor_9::ClaimGenerator,
    ) -> (
        ComponentTrace<N_TRACE_COLUMNS>,
        Claim,
        InteractionClaimGenerator,
    ) {
        let mut packed_inputs = self.packed_inputs.into_inner().unwrap();
        assert!(!packed_inputs.is_empty());
        assert!(self.remainder_inputs.lock().unwrap().is_empty());
        let n_vec_rows = packed_inputs.len();
        let n_rows = n_vec_rows * N_LANES;
        let packed_size = n_vec_rows.next_power_of_two();
        let log_size = packed_size.ilog2() + LOG_N_LANES;
        packed_inputs.resize(packed_size, *packed_inputs.first().unwrap());

        let (trace, lookup_data, sub_component_inputs) = write_trace_simd(
            packed_inputs,
            n_rows,
            verify_bitwise_xor_8_state,
            verify_bitwise_xor_12_state,
            verify_bitwise_xor_4_state,
            verify_bitwise_xor_7_state,
            verify_bitwise_xor_9_state,
        );
        for inputs in sub_component_inputs.verify_bitwise_xor_8 {
            add_inputs(
                verify_bitwise_xor_8_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_8_b {
            add_inputs(
                verify_bitwise_xor_8_state,
                &inputs,
                inputs.len() * N_LANES,
                1,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_12 {
            add_inputs(
                verify_bitwise_xor_12_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_4 {
            add_inputs(
                verify_bitwise_xor_4_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_7 {
            add_inputs(
                verify_bitwise_xor_7_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_9 {
            add_inputs(
                verify_bitwise_xor_9_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }

        (
            trace,
            Claim { log_size },
            InteractionClaimGenerator {
                log_size,
                lookup_data,
            },
        )
    }
}

impl AddInputs for ClaimGenerator {
    type PackedInputType = PackedInputType;
    type InputType = InputType;

    fn add_packed_inputs(&self, inputs: &[PackedInputType], _relation_index: usize) {
        self.packed_inputs.lock().unwrap().extend(inputs);
    }
    fn add_input(&self, input: &InputType, _relation_index: usize) {
        self.remainder_inputs.lock().unwrap().push(*input);
    }
}

#[derive(Uninitialized, IterMut, ParIterMut)]
struct SubComponentInputs {
    verify_bitwise_xor_8: [Vec<verify_bitwise_xor_8::PackedInputType>; 4],
    verify_bitwise_xor_8_b: [Vec<verify_bitwise_xor_8::PackedInputType>; 4],
    verify_bitwise_xor_12: [Vec<verify_bitwise_xor_12::PackedInputType>; 2],
    verify_bitwise_xor_4: [Vec<verify_bitwise_xor_4::PackedInputType>; 2],
    verify_bitwise_xor_7: [Vec<verify_bitwise_xor_7::PackedInputType>; 2],
    verify_bitwise_xor_9: [Vec<verify_bitwise_xor_9::PackedInputType>; 2],
}

#[allow(clippy::useless_conversion)]
#[allow(unused_variables)]
#[allow(clippy::double_parens)]
#[allow(non_snake_case)]
fn write_trace_simd(
    inputs: Vec<PackedInputType>,
    n_rows: usize,
    verify_bitwise_xor_8_state: &verify_bitwise_xor_8::ClaimGenerator,
    verify_bitwise_xor_12_state: &verify_bitwise_xor_12::ClaimGenerator,
    verify_bitwise_xor_4_state: &verify_bitwise_xor_4::ClaimGenerator,
    verify_bitwise_xor_7_state: &verify_bitwise_xor_7::ClaimGenerator,
    verify_bitwise_xor_9_state: &verify_bitwise_xor_9::ClaimGenerator,
) -> (
    ComponentTrace<N_TRACE_COLUMNS>,
    LookupData,
    SubComponentInputs,
) {
    let log_n_packed_rows = inputs.len().ilog2();
    let log_size = log_n_packed_rows + LOG_N_LANES;
    let (mut trace, mut lookup_data, mut sub_component_inputs) = unsafe {
        (
            ComponentTrace::<N_TRACE_COLUMNS>::uninitialized(log_size),
            LookupData::uninitialized(log_n_packed_rows),
            SubComponentInputs::uninitialized(log_n_packed_rows),
        )
    };

    let M31_1 = PackedM31::broadcast(M31::from(1));
    let M31_112558620 = PackedM31::broadcast(M31::from(112558620));
    let M31_1139985212 = PackedM31::broadcast(M31::from(1139985212));
    let M31_128 = PackedM31::broadcast(M31::from(128));
    let M31_16 = PackedM31::broadcast(M31::from(16));
    let M31_256 = PackedM31::broadcast(M31::from(256));
    let M31_4096 = PackedM31::broadcast(M31::from(4096));
    let M31_45448144 = PackedM31::broadcast(M31::from(45448144));
    let M31_512 = PackedM31::broadcast(M31::from(512));
    let M31_521092554 = PackedM31::broadcast(M31::from(521092554));
    let M31_62225763 = PackedM31::broadcast(M31::from(62225763));
    let M31_648362599 = PackedM31::broadcast(M31::from(648362599));
    let M31_95781001 = PackedM31::broadcast(M31::from(95781001));
    let UInt16_12 = PackedUInt16::broadcast(UInt16::from(12));
    let UInt16_7 = PackedUInt16::broadcast(UInt16::from(7));
    let UInt16_8 = PackedUInt16::broadcast(UInt16::from(8));
    let UInt32_0 = PackedUInt32::broadcast(UInt32::from(0));
    let enabler_col = Enabler::new(n_rows);

    (
        trace.par_iter_mut(),
        lookup_data.par_iter_mut(),
        sub_component_inputs.par_iter_mut(),
        inputs.into_par_iter(),
    )
        .into_par_iter()
        .enumerate()
        .for_each(
            |(row_index, (row, lookup_data, sub_component_inputs, blake_g_input))| {
                let input_limb_0_col0 = blake_g_input[0].low().as_m31();
                *row[0] = input_limb_0_col0;
                let input_limb_1_col1 = blake_g_input[0].high().as_m31();
                *row[1] = input_limb_1_col1;
                let input_limb_2_col2 = blake_g_input[1].low().as_m31();
                *row[2] = input_limb_2_col2;
                let input_limb_3_col3 = blake_g_input[1].high().as_m31();
                *row[3] = input_limb_3_col3;
                let input_limb_4_col4 = blake_g_input[2].low().as_m31();
                *row[4] = input_limb_4_col4;
                let input_limb_5_col5 = blake_g_input[2].high().as_m31();
                *row[5] = input_limb_5_col5;
                let input_limb_6_col6 = blake_g_input[3].low().as_m31();
                *row[6] = input_limb_6_col6;
                let input_limb_7_col7 = blake_g_input[3].high().as_m31();
                *row[7] = input_limb_7_col7;
                let input_limb_8_col8 = blake_g_input[4].low().as_m31();
                *row[8] = input_limb_8_col8;
                let input_limb_9_col9 = blake_g_input[4].high().as_m31();
                *row[9] = input_limb_9_col9;
                let input_limb_10_col10 = blake_g_input[5].low().as_m31();
                *row[10] = input_limb_10_col10;
                let input_limb_11_col11 = blake_g_input[5].high().as_m31();
                *row[11] = input_limb_11_col11;

                // Triple Sum 32.

                let triple_sum32_res_tmp_4da06_0 =
                    (((blake_g_input[0]) + (blake_g_input[1])) + (blake_g_input[4]));
                let triple_sum32_res_limb_0_col12 = triple_sum32_res_tmp_4da06_0.low().as_m31();
                *row[12] = triple_sum32_res_limb_0_col12;
                let triple_sum32_res_limb_1_col13 = triple_sum32_res_tmp_4da06_0.high().as_m31();
                *row[13] = triple_sum32_res_limb_1_col13;
                let triple_sum_32_output_tmp_4da06_3 = triple_sum32_res_tmp_4da06_0;

                // Xor Rot 32 R 16.

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_4 =
                    ((triple_sum_32_output_tmp_4da06_3.low()) >> (UInt16_8));
                let ms_8_bits_col14 = ms_8_bits_tmp_4da06_4.as_m31();
                *row[14] = ms_8_bits_col14;
                let split_16_low_part_size_8_output_tmp_4da06_5 = [
                    ((triple_sum32_res_limb_0_col12) - ((ms_8_bits_col14) * (M31_256))),
                    ms_8_bits_col14,
                ];

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_6 =
                    ((triple_sum_32_output_tmp_4da06_3.high()) >> (UInt16_8));
                let ms_8_bits_col15 = ms_8_bits_tmp_4da06_6.as_m31();
                *row[15] = ms_8_bits_col15;
                let split_16_low_part_size_8_output_tmp_4da06_7 = [
                    ((triple_sum32_res_limb_1_col13) - ((ms_8_bits_col15) * (M31_256))),
                    ms_8_bits_col15,
                ];

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_8 = ((blake_g_input[3].low()) >> (UInt16_8));
                let ms_8_bits_col16 = ms_8_bits_tmp_4da06_8.as_m31();
                *row[16] = ms_8_bits_col16;
                let split_16_low_part_size_8_output_tmp_4da06_9 = [
                    ((input_limb_6_col6) - ((ms_8_bits_col16) * (M31_256))),
                    ms_8_bits_col16,
                ];

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_10 = ((blake_g_input[3].high()) >> (UInt16_8));
                let ms_8_bits_col17 = ms_8_bits_tmp_4da06_10.as_m31();
                *row[17] = ms_8_bits_col17;
                let split_16_low_part_size_8_output_tmp_4da06_11 = [
                    ((input_limb_7_col7) - ((ms_8_bits_col17) * (M31_256))),
                    ms_8_bits_col17,
                ];

                // Bitwise Xor Num Bits 8.

                let xor_tmp_4da06_12 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_8_output_tmp_4da06_5[0]))
                        ^ (PackedUInt16::from_m31(split_16_low_part_size_8_output_tmp_4da06_9[0])));
                let xor_col18 = xor_tmp_4da06_12.as_m31();
                *row[18] = xor_col18;
                *sub_component_inputs.verify_bitwise_xor_8[0] = [
                    split_16_low_part_size_8_output_tmp_4da06_5[0],
                    split_16_low_part_size_8_output_tmp_4da06_9[0],
                    xor_col18,
                ];
                *lookup_data.verify_bitwise_xor_8_0 = [
                    M31_112558620,
                    split_16_low_part_size_8_output_tmp_4da06_5[0],
                    split_16_low_part_size_8_output_tmp_4da06_9[0],
                    xor_col18,
                ];

                // Bitwise Xor Num Bits 8.

                let xor_tmp_4da06_14 = ((PackedUInt16::from_m31(ms_8_bits_col14))
                    ^ (PackedUInt16::from_m31(ms_8_bits_col16)));
                let xor_col19 = xor_tmp_4da06_14.as_m31();
                *row[19] = xor_col19;
                *sub_component_inputs.verify_bitwise_xor_8[1] =
                    [ms_8_bits_col14, ms_8_bits_col16, xor_col19];
                *lookup_data.verify_bitwise_xor_8_1 =
                    [M31_112558620, ms_8_bits_col14, ms_8_bits_col16, xor_col19];

                // Bitwise Xor Num Bits 8 B.

                let xor_tmp_4da06_16 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_8_output_tmp_4da06_7[0]))
                        ^ (PackedUInt16::from_m31(
                            split_16_low_part_size_8_output_tmp_4da06_11[0],
                        )));
                let xor_col20 = xor_tmp_4da06_16.as_m31();
                *row[20] = xor_col20;
                *sub_component_inputs.verify_bitwise_xor_8_b[0] = [
                    split_16_low_part_size_8_output_tmp_4da06_7[0],
                    split_16_low_part_size_8_output_tmp_4da06_11[0],
                    xor_col20,
                ];
                *lookup_data.verify_bitwise_xor_8_b_2 = [
                    M31_521092554,
                    split_16_low_part_size_8_output_tmp_4da06_7[0],
                    split_16_low_part_size_8_output_tmp_4da06_11[0],
                    xor_col20,
                ];

                // Bitwise Xor Num Bits 8 B.

                let xor_tmp_4da06_18 = ((PackedUInt16::from_m31(ms_8_bits_col15))
                    ^ (PackedUInt16::from_m31(ms_8_bits_col17)));
                let xor_col21 = xor_tmp_4da06_18.as_m31();
                *row[21] = xor_col21;
                *sub_component_inputs.verify_bitwise_xor_8_b[1] =
                    [ms_8_bits_col15, ms_8_bits_col17, xor_col21];
                *lookup_data.verify_bitwise_xor_8_b_3 =
                    [M31_521092554, ms_8_bits_col15, ms_8_bits_col17, xor_col21];

                let xor_rot_16_output_tmp_4da06_20 = PackedUInt32::from_limbs([
                    ((xor_col20) + ((xor_col21) * (M31_256))),
                    ((xor_col18) + ((xor_col19) * (M31_256))),
                ]);
                let xor_rot_32_r_16_output_tmp_4da06_21 = xor_rot_16_output_tmp_4da06_20;

                // Triple Sum 32.

                let triple_sum32_res_tmp_4da06_22 =
                    (((blake_g_input[2]) + (xor_rot_32_r_16_output_tmp_4da06_21)) + (UInt32_0));
                let triple_sum32_res_limb_0_col22 = triple_sum32_res_tmp_4da06_22.low().as_m31();
                *row[22] = triple_sum32_res_limb_0_col22;
                let triple_sum32_res_limb_1_col23 = triple_sum32_res_tmp_4da06_22.high().as_m31();
                *row[23] = triple_sum32_res_limb_1_col23;
                let triple_sum_32_output_tmp_4da06_25 = triple_sum32_res_tmp_4da06_22;

                // Xor Rot 32 R 12.

                // Split 16 Low Part Size 12.

                let ms_4_bits_tmp_4da06_26 = ((blake_g_input[1].low()) >> (UInt16_12));
                let ms_4_bits_col24 = ms_4_bits_tmp_4da06_26.as_m31();
                *row[24] = ms_4_bits_col24;
                let split_16_low_part_size_12_output_tmp_4da06_27 = [
                    ((input_limb_2_col2) - ((ms_4_bits_col24) * (M31_4096))),
                    ms_4_bits_col24,
                ];

                // Split 16 Low Part Size 12.

                let ms_4_bits_tmp_4da06_28 = ((blake_g_input[1].high()) >> (UInt16_12));
                let ms_4_bits_col25 = ms_4_bits_tmp_4da06_28.as_m31();
                *row[25] = ms_4_bits_col25;
                let split_16_low_part_size_12_output_tmp_4da06_29 = [
                    ((input_limb_3_col3) - ((ms_4_bits_col25) * (M31_4096))),
                    ms_4_bits_col25,
                ];

                // Split 16 Low Part Size 12.

                let ms_4_bits_tmp_4da06_30 =
                    ((triple_sum_32_output_tmp_4da06_25.low()) >> (UInt16_12));
                let ms_4_bits_col26 = ms_4_bits_tmp_4da06_30.as_m31();
                *row[26] = ms_4_bits_col26;
                let split_16_low_part_size_12_output_tmp_4da06_31 = [
                    ((triple_sum32_res_limb_0_col22) - ((ms_4_bits_col26) * (M31_4096))),
                    ms_4_bits_col26,
                ];

                // Split 16 Low Part Size 12.

                let ms_4_bits_tmp_4da06_32 =
                    ((triple_sum_32_output_tmp_4da06_25.high()) >> (UInt16_12));
                let ms_4_bits_col27 = ms_4_bits_tmp_4da06_32.as_m31();
                *row[27] = ms_4_bits_col27;
                let split_16_low_part_size_12_output_tmp_4da06_33 = [
                    ((triple_sum32_res_limb_1_col23) - ((ms_4_bits_col27) * (M31_4096))),
                    ms_4_bits_col27,
                ];

                // Bitwise Xor Num Bits 12.

                let xor_tmp_4da06_34 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_12_output_tmp_4da06_27[0]))
                        ^ (PackedUInt16::from_m31(
                            split_16_low_part_size_12_output_tmp_4da06_31[0],
                        )));
                let xor_col28 = xor_tmp_4da06_34.as_m31();
                *row[28] = xor_col28;
                *sub_component_inputs.verify_bitwise_xor_12[0] = [
                    split_16_low_part_size_12_output_tmp_4da06_27[0],
                    split_16_low_part_size_12_output_tmp_4da06_31[0],
                    xor_col28,
                ];
                *lookup_data.verify_bitwise_xor_12_4 = [
                    M31_648362599,
                    split_16_low_part_size_12_output_tmp_4da06_27[0],
                    split_16_low_part_size_12_output_tmp_4da06_31[0],
                    xor_col28,
                ];

                // Bitwise Xor Num Bits 4.

                let xor_tmp_4da06_36 = ((PackedUInt16::from_m31(ms_4_bits_col24))
                    ^ (PackedUInt16::from_m31(ms_4_bits_col26)));
                let xor_col29 = xor_tmp_4da06_36.as_m31();
                *row[29] = xor_col29;
                *sub_component_inputs.verify_bitwise_xor_4[0] =
                    [ms_4_bits_col24, ms_4_bits_col26, xor_col29];
                *lookup_data.verify_bitwise_xor_4_5 =
                    [M31_45448144, ms_4_bits_col24, ms_4_bits_col26, xor_col29];

                // Bitwise Xor Num Bits 12.

                let xor_tmp_4da06_38 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_12_output_tmp_4da06_29[0]))
                        ^ (PackedUInt16::from_m31(
                            split_16_low_part_size_12_output_tmp_4da06_33[0],
                        )));
                let xor_col30 = xor_tmp_4da06_38.as_m31();
                *row[30] = xor_col30;
                *sub_component_inputs.verify_bitwise_xor_12[1] = [
                    split_16_low_part_size_12_output_tmp_4da06_29[0],
                    split_16_low_part_size_12_output_tmp_4da06_33[0],
                    xor_col30,
                ];
                *lookup_data.verify_bitwise_xor_12_6 = [
                    M31_648362599,
                    split_16_low_part_size_12_output_tmp_4da06_29[0],
                    split_16_low_part_size_12_output_tmp_4da06_33[0],
                    xor_col30,
                ];

                // Bitwise Xor Num Bits 4.

                let xor_tmp_4da06_40 = ((PackedUInt16::from_m31(ms_4_bits_col25))
                    ^ (PackedUInt16::from_m31(ms_4_bits_col27)));
                let xor_col31 = xor_tmp_4da06_40.as_m31();
                *row[31] = xor_col31;
                *sub_component_inputs.verify_bitwise_xor_4[1] =
                    [ms_4_bits_col25, ms_4_bits_col27, xor_col31];
                *lookup_data.verify_bitwise_xor_4_7 =
                    [M31_45448144, ms_4_bits_col25, ms_4_bits_col27, xor_col31];

                let xor_rot_12_output_tmp_4da06_42 = PackedUInt32::from_limbs([
                    ((xor_col29) + ((xor_col30) * (M31_16))),
                    ((xor_col31) + ((xor_col28) * (M31_16))),
                ]);
                let xor_rot_32_r_12_output_tmp_4da06_43 = xor_rot_12_output_tmp_4da06_42;

                // Triple Sum 32.

                let triple_sum32_res_tmp_4da06_44 = (((triple_sum_32_output_tmp_4da06_3)
                    + (xor_rot_32_r_12_output_tmp_4da06_43))
                    + (blake_g_input[5]));
                let triple_sum32_res_limb_0_col32 = triple_sum32_res_tmp_4da06_44.low().as_m31();
                *row[32] = triple_sum32_res_limb_0_col32;
                let triple_sum32_res_limb_1_col33 = triple_sum32_res_tmp_4da06_44.high().as_m31();
                *row[33] = triple_sum32_res_limb_1_col33;
                let triple_sum_32_output_tmp_4da06_47 = triple_sum32_res_tmp_4da06_44;

                // Xor Rot 32 R 8.

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_48 =
                    ((triple_sum_32_output_tmp_4da06_47.low()) >> (UInt16_8));
                let ms_8_bits_col34 = ms_8_bits_tmp_4da06_48.as_m31();
                *row[34] = ms_8_bits_col34;
                let split_16_low_part_size_8_output_tmp_4da06_49 = [
                    ((triple_sum32_res_limb_0_col32) - ((ms_8_bits_col34) * (M31_256))),
                    ms_8_bits_col34,
                ];

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_50 =
                    ((triple_sum_32_output_tmp_4da06_47.high()) >> (UInt16_8));
                let ms_8_bits_col35 = ms_8_bits_tmp_4da06_50.as_m31();
                *row[35] = ms_8_bits_col35;
                let split_16_low_part_size_8_output_tmp_4da06_51 = [
                    ((triple_sum32_res_limb_1_col33) - ((ms_8_bits_col35) * (M31_256))),
                    ms_8_bits_col35,
                ];

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_52 =
                    ((xor_rot_32_r_16_output_tmp_4da06_21.low()) >> (UInt16_8));
                let ms_8_bits_col36 = ms_8_bits_tmp_4da06_52.as_m31();
                *row[36] = ms_8_bits_col36;
                let split_16_low_part_size_8_output_tmp_4da06_53 = [
                    ((xor_rot_16_output_tmp_4da06_20.low().as_m31())
                        - ((ms_8_bits_col36) * (M31_256))),
                    ms_8_bits_col36,
                ];

                // Split 16 Low Part Size 8.

                let ms_8_bits_tmp_4da06_54 =
                    ((xor_rot_32_r_16_output_tmp_4da06_21.high()) >> (UInt16_8));
                let ms_8_bits_col37 = ms_8_bits_tmp_4da06_54.as_m31();
                *row[37] = ms_8_bits_col37;
                let split_16_low_part_size_8_output_tmp_4da06_55 = [
                    ((xor_rot_16_output_tmp_4da06_20.high().as_m31())
                        - ((ms_8_bits_col37) * (M31_256))),
                    ms_8_bits_col37,
                ];

                // Bitwise Xor Num Bits 8.

                let xor_tmp_4da06_56 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_8_output_tmp_4da06_49[0]))
                        ^ (PackedUInt16::from_m31(
                            split_16_low_part_size_8_output_tmp_4da06_53[0],
                        )));
                let xor_col38 = xor_tmp_4da06_56.as_m31();
                *row[38] = xor_col38;
                *sub_component_inputs.verify_bitwise_xor_8[2] = [
                    split_16_low_part_size_8_output_tmp_4da06_49[0],
                    split_16_low_part_size_8_output_tmp_4da06_53[0],
                    xor_col38,
                ];
                *lookup_data.verify_bitwise_xor_8_8 = [
                    M31_112558620,
                    split_16_low_part_size_8_output_tmp_4da06_49[0],
                    split_16_low_part_size_8_output_tmp_4da06_53[0],
                    xor_col38,
                ];

                // Bitwise Xor Num Bits 8.

                let xor_tmp_4da06_58 = ((PackedUInt16::from_m31(ms_8_bits_col34))
                    ^ (PackedUInt16::from_m31(ms_8_bits_col36)));
                let xor_col39 = xor_tmp_4da06_58.as_m31();
                *row[39] = xor_col39;
                *sub_component_inputs.verify_bitwise_xor_8[3] =
                    [ms_8_bits_col34, ms_8_bits_col36, xor_col39];
                *lookup_data.verify_bitwise_xor_8_9 =
                    [M31_112558620, ms_8_bits_col34, ms_8_bits_col36, xor_col39];

                // Bitwise Xor Num Bits 8 B.

                let xor_tmp_4da06_60 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_8_output_tmp_4da06_51[0]))
                        ^ (PackedUInt16::from_m31(
                            split_16_low_part_size_8_output_tmp_4da06_55[0],
                        )));
                let xor_col40 = xor_tmp_4da06_60.as_m31();
                *row[40] = xor_col40;
                *sub_component_inputs.verify_bitwise_xor_8_b[2] = [
                    split_16_low_part_size_8_output_tmp_4da06_51[0],
                    split_16_low_part_size_8_output_tmp_4da06_55[0],
                    xor_col40,
                ];
                *lookup_data.verify_bitwise_xor_8_b_10 = [
                    M31_521092554,
                    split_16_low_part_size_8_output_tmp_4da06_51[0],
                    split_16_low_part_size_8_output_tmp_4da06_55[0],
                    xor_col40,
                ];

                // Bitwise Xor Num Bits 8 B.

                let xor_tmp_4da06_62 = ((PackedUInt16::from_m31(ms_8_bits_col35))
                    ^ (PackedUInt16::from_m31(ms_8_bits_col37)));
                let xor_col41 = xor_tmp_4da06_62.as_m31();
                *row[41] = xor_col41;
                *sub_component_inputs.verify_bitwise_xor_8_b[3] =
                    [ms_8_bits_col35, ms_8_bits_col37, xor_col41];
                *lookup_data.verify_bitwise_xor_8_b_11 =
                    [M31_521092554, ms_8_bits_col35, ms_8_bits_col37, xor_col41];

                let xor_rot_8_output_tmp_4da06_64 = PackedUInt32::from_limbs([
                    ((xor_col39) + ((xor_col40) * (M31_256))),
                    ((xor_col41) + ((xor_col38) * (M31_256))),
                ]);
                let xor_rot_32_r_8_output_tmp_4da06_65 = xor_rot_8_output_tmp_4da06_64;

                // Triple Sum 32.

                let triple_sum32_res_tmp_4da06_66 = (((triple_sum_32_output_tmp_4da06_25)
                    + (xor_rot_32_r_8_output_tmp_4da06_65))
                    + (UInt32_0));
                let triple_sum32_res_limb_0_col42 = triple_sum32_res_tmp_4da06_66.low().as_m31();
                *row[42] = triple_sum32_res_limb_0_col42;
                let triple_sum32_res_limb_1_col43 = triple_sum32_res_tmp_4da06_66.high().as_m31();
                *row[43] = triple_sum32_res_limb_1_col43;
                let triple_sum_32_output_tmp_4da06_69 = triple_sum32_res_tmp_4da06_66;

                // Xor Rot 32 R 7.

                // Split 16 Low Part Size 7.

                let ms_9_bits_tmp_4da06_70 =
                    ((xor_rot_32_r_12_output_tmp_4da06_43.low()) >> (UInt16_7));
                let ms_9_bits_col44 = ms_9_bits_tmp_4da06_70.as_m31();
                *row[44] = ms_9_bits_col44;
                let split_16_low_part_size_7_output_tmp_4da06_71 = [
                    ((xor_rot_12_output_tmp_4da06_42.low().as_m31())
                        - ((ms_9_bits_col44) * (M31_128))),
                    ms_9_bits_col44,
                ];

                // Split 16 Low Part Size 7.

                let ms_9_bits_tmp_4da06_72 =
                    ((xor_rot_32_r_12_output_tmp_4da06_43.high()) >> (UInt16_7));
                let ms_9_bits_col45 = ms_9_bits_tmp_4da06_72.as_m31();
                *row[45] = ms_9_bits_col45;
                let split_16_low_part_size_7_output_tmp_4da06_73 = [
                    ((xor_rot_12_output_tmp_4da06_42.high().as_m31())
                        - ((ms_9_bits_col45) * (M31_128))),
                    ms_9_bits_col45,
                ];

                // Split 16 Low Part Size 7.

                let ms_9_bits_tmp_4da06_74 =
                    ((triple_sum_32_output_tmp_4da06_69.low()) >> (UInt16_7));
                let ms_9_bits_col46 = ms_9_bits_tmp_4da06_74.as_m31();
                *row[46] = ms_9_bits_col46;
                let split_16_low_part_size_7_output_tmp_4da06_75 = [
                    ((triple_sum32_res_limb_0_col42) - ((ms_9_bits_col46) * (M31_128))),
                    ms_9_bits_col46,
                ];

                // Split 16 Low Part Size 7.

                let ms_9_bits_tmp_4da06_76 =
                    ((triple_sum_32_output_tmp_4da06_69.high()) >> (UInt16_7));
                let ms_9_bits_col47 = ms_9_bits_tmp_4da06_76.as_m31();
                *row[47] = ms_9_bits_col47;
                let split_16_low_part_size_7_output_tmp_4da06_77 = [
                    ((triple_sum32_res_limb_1_col43) - ((ms_9_bits_col47) * (M31_128))),
                    ms_9_bits_col47,
                ];

                // Bitwise Xor Num Bits 7.

                let xor_tmp_4da06_78 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_7_output_tmp_4da06_71[0]))
                        ^ (PackedUInt16::from_m31(
                            split_16_low_part_size_7_output_tmp_4da06_75[0],
                        )));
                let xor_col48 = xor_tmp_4da06_78.as_m31();
                *row[48] = xor_col48;
                *sub_component_inputs.verify_bitwise_xor_7[0] = [
                    split_16_low_part_size_7_output_tmp_4da06_71[0],
                    split_16_low_part_size_7_output_tmp_4da06_75[0],
                    xor_col48,
                ];
                *lookup_data.verify_bitwise_xor_7_12 = [
                    M31_62225763,
                    split_16_low_part_size_7_output_tmp_4da06_71[0],
                    split_16_low_part_size_7_output_tmp_4da06_75[0],
                    xor_col48,
                ];

                // Bitwise Xor Num Bits 9.

                let xor_tmp_4da06_80 = ((PackedUInt16::from_m31(ms_9_bits_col44))
                    ^ (PackedUInt16::from_m31(ms_9_bits_col46)));
                let xor_col49 = xor_tmp_4da06_80.as_m31();
                *row[49] = xor_col49;
                *sub_component_inputs.verify_bitwise_xor_9[0] =
                    [ms_9_bits_col44, ms_9_bits_col46, xor_col49];
                *lookup_data.verify_bitwise_xor_9_13 =
                    [M31_95781001, ms_9_bits_col44, ms_9_bits_col46, xor_col49];

                // Bitwise Xor Num Bits 7.

                let xor_tmp_4da06_82 =
                    ((PackedUInt16::from_m31(split_16_low_part_size_7_output_tmp_4da06_73[0]))
                        ^ (PackedUInt16::from_m31(
                            split_16_low_part_size_7_output_tmp_4da06_77[0],
                        )));
                let xor_col50 = xor_tmp_4da06_82.as_m31();
                *row[50] = xor_col50;
                *sub_component_inputs.verify_bitwise_xor_7[1] = [
                    split_16_low_part_size_7_output_tmp_4da06_73[0],
                    split_16_low_part_size_7_output_tmp_4da06_77[0],
                    xor_col50,
                ];
                *lookup_data.verify_bitwise_xor_7_14 = [
                    M31_62225763,
                    split_16_low_part_size_7_output_tmp_4da06_73[0],
                    split_16_low_part_size_7_output_tmp_4da06_77[0],
                    xor_col50,
                ];

                // Bitwise Xor Num Bits 9.

                let xor_tmp_4da06_84 = ((PackedUInt16::from_m31(ms_9_bits_col45))
                    ^ (PackedUInt16::from_m31(ms_9_bits_col47)));
                let xor_col51 = xor_tmp_4da06_84.as_m31();
                *row[51] = xor_col51;
                *sub_component_inputs.verify_bitwise_xor_9[1] =
                    [ms_9_bits_col45, ms_9_bits_col47, xor_col51];
                *lookup_data.verify_bitwise_xor_9_15 =
                    [M31_95781001, ms_9_bits_col45, ms_9_bits_col47, xor_col51];

                let xor_rot_7_output_tmp_4da06_86 = PackedUInt32::from_limbs([
                    ((xor_col49) + ((xor_col50) * (M31_512))),
                    ((xor_col51) + ((xor_col48) * (M31_512))),
                ]);
                let xor_rot_32_r_7_output_tmp_4da06_87 = xor_rot_7_output_tmp_4da06_86;

                let enabler_col52 = enabler_col.packed_at(row_index);
                *row[52] = enabler_col52;
                *lookup_data.blake_g_16 = [
                    M31_1139985212,
                    input_limb_0_col0,
                    input_limb_1_col1,
                    input_limb_2_col2,
                    input_limb_3_col3,
                    input_limb_4_col4,
                    input_limb_5_col5,
                    input_limb_6_col6,
                    input_limb_7_col7,
                    input_limb_8_col8,
                    input_limb_9_col9,
                    input_limb_10_col10,
                    input_limb_11_col11,
                    triple_sum32_res_limb_0_col32,
                    triple_sum32_res_limb_1_col33,
                    xor_rot_7_output_tmp_4da06_86.low().as_m31(),
                    xor_rot_7_output_tmp_4da06_86.high().as_m31(),
                    triple_sum32_res_limb_0_col42,
                    triple_sum32_res_limb_1_col43,
                    xor_rot_8_output_tmp_4da06_64.low().as_m31(),
                    xor_rot_8_output_tmp_4da06_64.high().as_m31(),
                ];
                *lookup_data.mults_0 = M31_1;
                *lookup_data.mults_1 = enabler_col52;
            },
        );

    (trace, lookup_data, sub_component_inputs)
}

// === BEGIN witness_genericize (generated; re-runnable) ===
//
// GENERATED by tools/witness_genericize for `blake_g` — mechanical rewrite of
// `write_trace_simd`'s per-row closure into a generic body over `WitnessEval`. Do not
// edit by hand: re-run the tool after upstream regeneration (this block is stripped and
// re-emitted idempotently). The original `write_trace_simd` above is the untouched
// byte-equality baseline (see `witness_eval::differential_test`).
//
// Flat layouts (derived, DECLARATION order):
//   LOOKUP words:
//     verify_bitwise_xor_8_0[4] 0..3
//     verify_bitwise_xor_8_1[4] 4..7
//     verify_bitwise_xor_8_b_2[4] 8..11
//     verify_bitwise_xor_8_b_3[4] 12..15
//     verify_bitwise_xor_12_4[4] 16..19
//     verify_bitwise_xor_4_5[4] 20..23
//     verify_bitwise_xor_12_6[4] 24..27
//     verify_bitwise_xor_4_7[4] 28..31
//     verify_bitwise_xor_8_8[4] 32..35
//     verify_bitwise_xor_8_9[4] 36..39
//     verify_bitwise_xor_8_b_10[4] 40..43
//     verify_bitwise_xor_8_b_11[4] 44..47
//     verify_bitwise_xor_7_12[4] 48..51
//     verify_bitwise_xor_9_13[4] 52..55
//     verify_bitwise_xor_7_14[4] 56..59
//     verify_bitwise_xor_9_15[4] 60..63
//     blake_g_16[21] 64..84
//     mults_0 85
//     mults_1 86
//     (87 words)
//   SUB-INPUT words:
//     verify_bitwise_xor_8[0] 0..2
//     verify_bitwise_xor_8[1] 3..5
//     verify_bitwise_xor_8[2] 6..8
//     verify_bitwise_xor_8[3] 9..11
//     verify_bitwise_xor_8_b[0] 12..14
//     verify_bitwise_xor_8_b[1] 15..17
//     verify_bitwise_xor_8_b[2] 18..20
//     verify_bitwise_xor_8_b[3] 21..23
//     verify_bitwise_xor_12[0] 24..26
//     verify_bitwise_xor_12[1] 27..29
//     verify_bitwise_xor_4[0] 30..32
//     verify_bitwise_xor_4[1] 33..35
//     verify_bitwise_xor_7[0] 36..38
//     verify_bitwise_xor_7[1] 39..41
//     verify_bitwise_xor_9[0] 42..44
//     verify_bitwise_xor_9[1] 45..47
//     (48 words)
use crate::witness::witness_eval::recording::{RecordingOutput, RecordingWitnessEval};
use crate::witness::witness_eval::simd::SimdWitnessEval;
use crate::witness::witness_eval::WitnessEval;

pub(crate) const N_LOOKUP_WORDS: usize = 87;
pub(crate) const N_SUB_INPUT_WORDS: usize = 48;

/// The per-row `blake_g` base-trace body, routed through `WitnessEval`.
/// Mechanical transcription of `write_trace_simd`'s per-row closure (baseline above).
#[allow(clippy::identity_op)]
#[allow(clippy::erasing_op)]
#[allow(unused_variables)]
#[allow(dead_code)]
fn blake_g_row_body<E: WitnessEval>(eval: &mut E) {
    let m31_1 = eval.m31_const(1);
    let m31_16 = eval.m31_const(16);
    let m31_128 = eval.m31_const(128);
    let m31_256 = eval.m31_const(256);
    let m31_512 = eval.m31_const(512);
    let m31_4096 = eval.m31_const(4096);
    let m31_45448144 = eval.m31_const(45448144);
    let m31_62225763 = eval.m31_const(62225763);
    let m31_95781001 = eval.m31_const(95781001);
    let m31_112558620 = eval.m31_const(112558620);
    let m31_521092554 = eval.m31_const(521092554);
    let m31_648362599 = eval.m31_const(648362599);
    let m31_1139985212 = eval.m31_const(1139985212);
    let wg_v0 = eval.input_u32(0);
    let wg_v1 = eval.u32_low(wg_v0);
    let input_limb_0_col0 = eval.u16_as_m31(wg_v1);
    eval.set_col(0, input_limb_0_col0);
    let wg_v2 = eval.input_u32(0);
    let wg_v3 = eval.u32_high(wg_v2);
    let input_limb_1_col1 = eval.u16_as_m31(wg_v3);
    eval.set_col(1, input_limb_1_col1);
    let wg_v4 = eval.input_u32(1);
    let wg_v5 = eval.u32_low(wg_v4);
    let input_limb_2_col2 = eval.u16_as_m31(wg_v5);
    eval.set_col(2, input_limb_2_col2);
    let wg_v6 = eval.input_u32(1);
    let wg_v7 = eval.u32_high(wg_v6);
    let input_limb_3_col3 = eval.u16_as_m31(wg_v7);
    eval.set_col(3, input_limb_3_col3);
    let wg_v8 = eval.input_u32(2);
    let wg_v9 = eval.u32_low(wg_v8);
    let input_limb_4_col4 = eval.u16_as_m31(wg_v9);
    eval.set_col(4, input_limb_4_col4);
    let wg_v10 = eval.input_u32(2);
    let wg_v11 = eval.u32_high(wg_v10);
    let input_limb_5_col5 = eval.u16_as_m31(wg_v11);
    eval.set_col(5, input_limb_5_col5);
    let wg_v12 = eval.input_u32(3);
    let wg_v13 = eval.u32_low(wg_v12);
    let input_limb_6_col6 = eval.u16_as_m31(wg_v13);
    eval.set_col(6, input_limb_6_col6);
    let wg_v14 = eval.input_u32(3);
    let wg_v15 = eval.u32_high(wg_v14);
    let input_limb_7_col7 = eval.u16_as_m31(wg_v15);
    eval.set_col(7, input_limb_7_col7);
    let wg_v16 = eval.input_u32(4);
    let wg_v17 = eval.u32_low(wg_v16);
    let input_limb_8_col8 = eval.u16_as_m31(wg_v17);
    eval.set_col(8, input_limb_8_col8);
    let wg_v18 = eval.input_u32(4);
    let wg_v19 = eval.u32_high(wg_v18);
    let input_limb_9_col9 = eval.u16_as_m31(wg_v19);
    eval.set_col(9, input_limb_9_col9);
    let wg_v20 = eval.input_u32(5);
    let wg_v21 = eval.u32_low(wg_v20);
    let input_limb_10_col10 = eval.u16_as_m31(wg_v21);
    eval.set_col(10, input_limb_10_col10);
    let wg_v22 = eval.input_u32(5);
    let wg_v23 = eval.u32_high(wg_v22);
    let input_limb_11_col11 = eval.u16_as_m31(wg_v23);
    eval.set_col(11, input_limb_11_col11);
    let wg_v24 = eval.input_u32(0);
    let wg_v25 = eval.input_u32(1);
    let wg_v26 = eval.u32_add(wg_v24, wg_v25);
    let wg_v27 = eval.input_u32(4);
    let triple_sum32_res_tmp_4da06_0 = eval.u32_add(wg_v26, wg_v27);
    let wg_v28 = eval.u32_low(triple_sum32_res_tmp_4da06_0);
    let triple_sum32_res_limb_0_col12 = eval.u16_as_m31(wg_v28);
    eval.set_col(12, triple_sum32_res_limb_0_col12);
    let wg_v29 = eval.u32_high(triple_sum32_res_tmp_4da06_0);
    let triple_sum32_res_limb_1_col13 = eval.u16_as_m31(wg_v29);
    eval.set_col(13, triple_sum32_res_limb_1_col13);
    let triple_sum_32_output_tmp_4da06_3 = triple_sum32_res_tmp_4da06_0;
    let wg_v30 = eval.u32_low(triple_sum_32_output_tmp_4da06_3);
    let ms_8_bits_tmp_4da06_4 = eval.u16_shr(wg_v30, 8);
    let ms_8_bits_col14 = eval.u16_as_m31(ms_8_bits_tmp_4da06_4);
    eval.set_col(14, ms_8_bits_col14);
    let wg_v31 = eval.m31_mul(ms_8_bits_col14, m31_256);
    let wg_v32 = eval.m31_sub(triple_sum32_res_limb_0_col12, wg_v31);
    let split_16_low_part_size_8_output_tmp_4da06_5 = [wg_v32, ms_8_bits_col14];
    let wg_v33 = eval.u32_high(triple_sum_32_output_tmp_4da06_3);
    let ms_8_bits_tmp_4da06_6 = eval.u16_shr(wg_v33, 8);
    let ms_8_bits_col15 = eval.u16_as_m31(ms_8_bits_tmp_4da06_6);
    eval.set_col(15, ms_8_bits_col15);
    let wg_v34 = eval.m31_mul(ms_8_bits_col15, m31_256);
    let wg_v35 = eval.m31_sub(triple_sum32_res_limb_1_col13, wg_v34);
    let split_16_low_part_size_8_output_tmp_4da06_7 = [wg_v35, ms_8_bits_col15];
    let wg_v36 = eval.input_u32(3);
    let wg_v37 = eval.u32_low(wg_v36);
    let ms_8_bits_tmp_4da06_8 = eval.u16_shr(wg_v37, 8);
    let ms_8_bits_col16 = eval.u16_as_m31(ms_8_bits_tmp_4da06_8);
    eval.set_col(16, ms_8_bits_col16);
    let wg_v38 = eval.m31_mul(ms_8_bits_col16, m31_256);
    let wg_v39 = eval.m31_sub(input_limb_6_col6, wg_v38);
    let split_16_low_part_size_8_output_tmp_4da06_9 = [wg_v39, ms_8_bits_col16];
    let wg_v40 = eval.input_u32(3);
    let wg_v41 = eval.u32_high(wg_v40);
    let ms_8_bits_tmp_4da06_10 = eval.u16_shr(wg_v41, 8);
    let ms_8_bits_col17 = eval.u16_as_m31(ms_8_bits_tmp_4da06_10);
    eval.set_col(17, ms_8_bits_col17);
    let wg_v42 = eval.m31_mul(ms_8_bits_col17, m31_256);
    let wg_v43 = eval.m31_sub(input_limb_7_col7, wg_v42);
    let split_16_low_part_size_8_output_tmp_4da06_11 = [wg_v43, ms_8_bits_col17];
    let wg_v44 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_5[0]);
    let wg_v45 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_9[0]);
    let xor_tmp_4da06_12 = eval.u16_xor(wg_v44, wg_v45);
    let xor_col18 = eval.u16_as_m31(xor_tmp_4da06_12);
    eval.set_col(18, xor_col18);
    eval.set_sub_input_word(0, split_16_low_part_size_8_output_tmp_4da06_5[0]);
    eval.set_sub_input_word(1, split_16_low_part_size_8_output_tmp_4da06_9[0]);
    eval.set_sub_input_word(2, xor_col18);
    eval.set_lookup_word(0, m31_112558620);
    eval.set_lookup_word(1, split_16_low_part_size_8_output_tmp_4da06_5[0]);
    eval.set_lookup_word(2, split_16_low_part_size_8_output_tmp_4da06_9[0]);
    eval.set_lookup_word(3, xor_col18);
    let wg_v46 = eval.u16_from_m31(ms_8_bits_col14);
    let wg_v47 = eval.u16_from_m31(ms_8_bits_col16);
    let xor_tmp_4da06_14 = eval.u16_xor(wg_v46, wg_v47);
    let xor_col19 = eval.u16_as_m31(xor_tmp_4da06_14);
    eval.set_col(19, xor_col19);
    eval.set_sub_input_word(3, ms_8_bits_col14);
    eval.set_sub_input_word(4, ms_8_bits_col16);
    eval.set_sub_input_word(5, xor_col19);
    eval.set_lookup_word(4, m31_112558620);
    eval.set_lookup_word(5, ms_8_bits_col14);
    eval.set_lookup_word(6, ms_8_bits_col16);
    eval.set_lookup_word(7, xor_col19);
    let wg_v48 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_7[0]);
    let wg_v49 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_11[0]);
    let xor_tmp_4da06_16 = eval.u16_xor(wg_v48, wg_v49);
    let xor_col20 = eval.u16_as_m31(xor_tmp_4da06_16);
    eval.set_col(20, xor_col20);
    eval.set_sub_input_word(12, split_16_low_part_size_8_output_tmp_4da06_7[0]);
    eval.set_sub_input_word(13, split_16_low_part_size_8_output_tmp_4da06_11[0]);
    eval.set_sub_input_word(14, xor_col20);
    eval.set_lookup_word(8, m31_521092554);
    eval.set_lookup_word(9, split_16_low_part_size_8_output_tmp_4da06_7[0]);
    eval.set_lookup_word(10, split_16_low_part_size_8_output_tmp_4da06_11[0]);
    eval.set_lookup_word(11, xor_col20);
    let wg_v50 = eval.u16_from_m31(ms_8_bits_col15);
    let wg_v51 = eval.u16_from_m31(ms_8_bits_col17);
    let xor_tmp_4da06_18 = eval.u16_xor(wg_v50, wg_v51);
    let xor_col21 = eval.u16_as_m31(xor_tmp_4da06_18);
    eval.set_col(21, xor_col21);
    eval.set_sub_input_word(15, ms_8_bits_col15);
    eval.set_sub_input_word(16, ms_8_bits_col17);
    eval.set_sub_input_word(17, xor_col21);
    eval.set_lookup_word(12, m31_521092554);
    eval.set_lookup_word(13, ms_8_bits_col15);
    eval.set_lookup_word(14, ms_8_bits_col17);
    eval.set_lookup_word(15, xor_col21);
    let wg_v52 = eval.m31_mul(xor_col21, m31_256);
    let wg_v53 = eval.m31_add(xor_col20, wg_v52);
    let wg_v54 = eval.m31_mul(xor_col19, m31_256);
    let wg_v55 = eval.m31_add(xor_col18, wg_v54);
    let xor_rot_16_output_tmp_4da06_20 = eval.u32_from_limbs(wg_v53, wg_v55);
    let xor_rot_32_r_16_output_tmp_4da06_21 = xor_rot_16_output_tmp_4da06_20;
    let wg_v56 = eval.input_u32(2);
    let wg_v57 = eval.u32_add(wg_v56, xor_rot_32_r_16_output_tmp_4da06_21);
    let wg_v58 = eval.u32_const(0);
    let triple_sum32_res_tmp_4da06_22 = eval.u32_add(wg_v57, wg_v58);
    let wg_v59 = eval.u32_low(triple_sum32_res_tmp_4da06_22);
    let triple_sum32_res_limb_0_col22 = eval.u16_as_m31(wg_v59);
    eval.set_col(22, triple_sum32_res_limb_0_col22);
    let wg_v60 = eval.u32_high(triple_sum32_res_tmp_4da06_22);
    let triple_sum32_res_limb_1_col23 = eval.u16_as_m31(wg_v60);
    eval.set_col(23, triple_sum32_res_limb_1_col23);
    let triple_sum_32_output_tmp_4da06_25 = triple_sum32_res_tmp_4da06_22;
    let wg_v61 = eval.input_u32(1);
    let wg_v62 = eval.u32_low(wg_v61);
    let ms_4_bits_tmp_4da06_26 = eval.u16_shr(wg_v62, 12);
    let ms_4_bits_col24 = eval.u16_as_m31(ms_4_bits_tmp_4da06_26);
    eval.set_col(24, ms_4_bits_col24);
    let wg_v63 = eval.m31_mul(ms_4_bits_col24, m31_4096);
    let wg_v64 = eval.m31_sub(input_limb_2_col2, wg_v63);
    let split_16_low_part_size_12_output_tmp_4da06_27 = [wg_v64, ms_4_bits_col24];
    let wg_v65 = eval.input_u32(1);
    let wg_v66 = eval.u32_high(wg_v65);
    let ms_4_bits_tmp_4da06_28 = eval.u16_shr(wg_v66, 12);
    let ms_4_bits_col25 = eval.u16_as_m31(ms_4_bits_tmp_4da06_28);
    eval.set_col(25, ms_4_bits_col25);
    let wg_v67 = eval.m31_mul(ms_4_bits_col25, m31_4096);
    let wg_v68 = eval.m31_sub(input_limb_3_col3, wg_v67);
    let split_16_low_part_size_12_output_tmp_4da06_29 = [wg_v68, ms_4_bits_col25];
    let wg_v69 = eval.u32_low(triple_sum_32_output_tmp_4da06_25);
    let ms_4_bits_tmp_4da06_30 = eval.u16_shr(wg_v69, 12);
    let ms_4_bits_col26 = eval.u16_as_m31(ms_4_bits_tmp_4da06_30);
    eval.set_col(26, ms_4_bits_col26);
    let wg_v70 = eval.m31_mul(ms_4_bits_col26, m31_4096);
    let wg_v71 = eval.m31_sub(triple_sum32_res_limb_0_col22, wg_v70);
    let split_16_low_part_size_12_output_tmp_4da06_31 = [wg_v71, ms_4_bits_col26];
    let wg_v72 = eval.u32_high(triple_sum_32_output_tmp_4da06_25);
    let ms_4_bits_tmp_4da06_32 = eval.u16_shr(wg_v72, 12);
    let ms_4_bits_col27 = eval.u16_as_m31(ms_4_bits_tmp_4da06_32);
    eval.set_col(27, ms_4_bits_col27);
    let wg_v73 = eval.m31_mul(ms_4_bits_col27, m31_4096);
    let wg_v74 = eval.m31_sub(triple_sum32_res_limb_1_col23, wg_v73);
    let split_16_low_part_size_12_output_tmp_4da06_33 = [wg_v74, ms_4_bits_col27];
    let wg_v75 = eval.u16_from_m31(split_16_low_part_size_12_output_tmp_4da06_27[0]);
    let wg_v76 = eval.u16_from_m31(split_16_low_part_size_12_output_tmp_4da06_31[0]);
    let xor_tmp_4da06_34 = eval.u16_xor(wg_v75, wg_v76);
    let xor_col28 = eval.u16_as_m31(xor_tmp_4da06_34);
    eval.set_col(28, xor_col28);
    eval.set_sub_input_word(24, split_16_low_part_size_12_output_tmp_4da06_27[0]);
    eval.set_sub_input_word(25, split_16_low_part_size_12_output_tmp_4da06_31[0]);
    eval.set_sub_input_word(26, xor_col28);
    eval.set_lookup_word(16, m31_648362599);
    eval.set_lookup_word(17, split_16_low_part_size_12_output_tmp_4da06_27[0]);
    eval.set_lookup_word(18, split_16_low_part_size_12_output_tmp_4da06_31[0]);
    eval.set_lookup_word(19, xor_col28);
    let wg_v77 = eval.u16_from_m31(ms_4_bits_col24);
    let wg_v78 = eval.u16_from_m31(ms_4_bits_col26);
    let xor_tmp_4da06_36 = eval.u16_xor(wg_v77, wg_v78);
    let xor_col29 = eval.u16_as_m31(xor_tmp_4da06_36);
    eval.set_col(29, xor_col29);
    eval.set_sub_input_word(30, ms_4_bits_col24);
    eval.set_sub_input_word(31, ms_4_bits_col26);
    eval.set_sub_input_word(32, xor_col29);
    eval.set_lookup_word(20, m31_45448144);
    eval.set_lookup_word(21, ms_4_bits_col24);
    eval.set_lookup_word(22, ms_4_bits_col26);
    eval.set_lookup_word(23, xor_col29);
    let wg_v79 = eval.u16_from_m31(split_16_low_part_size_12_output_tmp_4da06_29[0]);
    let wg_v80 = eval.u16_from_m31(split_16_low_part_size_12_output_tmp_4da06_33[0]);
    let xor_tmp_4da06_38 = eval.u16_xor(wg_v79, wg_v80);
    let xor_col30 = eval.u16_as_m31(xor_tmp_4da06_38);
    eval.set_col(30, xor_col30);
    eval.set_sub_input_word(27, split_16_low_part_size_12_output_tmp_4da06_29[0]);
    eval.set_sub_input_word(28, split_16_low_part_size_12_output_tmp_4da06_33[0]);
    eval.set_sub_input_word(29, xor_col30);
    eval.set_lookup_word(24, m31_648362599);
    eval.set_lookup_word(25, split_16_low_part_size_12_output_tmp_4da06_29[0]);
    eval.set_lookup_word(26, split_16_low_part_size_12_output_tmp_4da06_33[0]);
    eval.set_lookup_word(27, xor_col30);
    let wg_v81 = eval.u16_from_m31(ms_4_bits_col25);
    let wg_v82 = eval.u16_from_m31(ms_4_bits_col27);
    let xor_tmp_4da06_40 = eval.u16_xor(wg_v81, wg_v82);
    let xor_col31 = eval.u16_as_m31(xor_tmp_4da06_40);
    eval.set_col(31, xor_col31);
    eval.set_sub_input_word(33, ms_4_bits_col25);
    eval.set_sub_input_word(34, ms_4_bits_col27);
    eval.set_sub_input_word(35, xor_col31);
    eval.set_lookup_word(28, m31_45448144);
    eval.set_lookup_word(29, ms_4_bits_col25);
    eval.set_lookup_word(30, ms_4_bits_col27);
    eval.set_lookup_word(31, xor_col31);
    let wg_v83 = eval.m31_mul(xor_col30, m31_16);
    let wg_v84 = eval.m31_add(xor_col29, wg_v83);
    let wg_v85 = eval.m31_mul(xor_col28, m31_16);
    let wg_v86 = eval.m31_add(xor_col31, wg_v85);
    let xor_rot_12_output_tmp_4da06_42 = eval.u32_from_limbs(wg_v84, wg_v86);
    let xor_rot_32_r_12_output_tmp_4da06_43 = xor_rot_12_output_tmp_4da06_42;
    let wg_v87 = eval.u32_add(
        triple_sum_32_output_tmp_4da06_3,
        xor_rot_32_r_12_output_tmp_4da06_43,
    );
    let wg_v88 = eval.input_u32(5);
    let triple_sum32_res_tmp_4da06_44 = eval.u32_add(wg_v87, wg_v88);
    let wg_v89 = eval.u32_low(triple_sum32_res_tmp_4da06_44);
    let triple_sum32_res_limb_0_col32 = eval.u16_as_m31(wg_v89);
    eval.set_col(32, triple_sum32_res_limb_0_col32);
    let wg_v90 = eval.u32_high(triple_sum32_res_tmp_4da06_44);
    let triple_sum32_res_limb_1_col33 = eval.u16_as_m31(wg_v90);
    eval.set_col(33, triple_sum32_res_limb_1_col33);
    let triple_sum_32_output_tmp_4da06_47 = triple_sum32_res_tmp_4da06_44;
    let wg_v91 = eval.u32_low(triple_sum_32_output_tmp_4da06_47);
    let ms_8_bits_tmp_4da06_48 = eval.u16_shr(wg_v91, 8);
    let ms_8_bits_col34 = eval.u16_as_m31(ms_8_bits_tmp_4da06_48);
    eval.set_col(34, ms_8_bits_col34);
    let wg_v92 = eval.m31_mul(ms_8_bits_col34, m31_256);
    let wg_v93 = eval.m31_sub(triple_sum32_res_limb_0_col32, wg_v92);
    let split_16_low_part_size_8_output_tmp_4da06_49 = [wg_v93, ms_8_bits_col34];
    let wg_v94 = eval.u32_high(triple_sum_32_output_tmp_4da06_47);
    let ms_8_bits_tmp_4da06_50 = eval.u16_shr(wg_v94, 8);
    let ms_8_bits_col35 = eval.u16_as_m31(ms_8_bits_tmp_4da06_50);
    eval.set_col(35, ms_8_bits_col35);
    let wg_v95 = eval.m31_mul(ms_8_bits_col35, m31_256);
    let wg_v96 = eval.m31_sub(triple_sum32_res_limb_1_col33, wg_v95);
    let split_16_low_part_size_8_output_tmp_4da06_51 = [wg_v96, ms_8_bits_col35];
    let wg_v97 = eval.u32_low(xor_rot_32_r_16_output_tmp_4da06_21);
    let ms_8_bits_tmp_4da06_52 = eval.u16_shr(wg_v97, 8);
    let ms_8_bits_col36 = eval.u16_as_m31(ms_8_bits_tmp_4da06_52);
    eval.set_col(36, ms_8_bits_col36);
    let wg_v98 = eval.u32_low(xor_rot_16_output_tmp_4da06_20);
    let wg_v99 = eval.u16_as_m31(wg_v98);
    let wg_v100 = eval.m31_mul(ms_8_bits_col36, m31_256);
    let wg_v101 = eval.m31_sub(wg_v99, wg_v100);
    let split_16_low_part_size_8_output_tmp_4da06_53 = [wg_v101, ms_8_bits_col36];
    let wg_v102 = eval.u32_high(xor_rot_32_r_16_output_tmp_4da06_21);
    let ms_8_bits_tmp_4da06_54 = eval.u16_shr(wg_v102, 8);
    let ms_8_bits_col37 = eval.u16_as_m31(ms_8_bits_tmp_4da06_54);
    eval.set_col(37, ms_8_bits_col37);
    let wg_v103 = eval.u32_high(xor_rot_16_output_tmp_4da06_20);
    let wg_v104 = eval.u16_as_m31(wg_v103);
    let wg_v105 = eval.m31_mul(ms_8_bits_col37, m31_256);
    let wg_v106 = eval.m31_sub(wg_v104, wg_v105);
    let split_16_low_part_size_8_output_tmp_4da06_55 = [wg_v106, ms_8_bits_col37];
    let wg_v107 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_49[0]);
    let wg_v108 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_53[0]);
    let xor_tmp_4da06_56 = eval.u16_xor(wg_v107, wg_v108);
    let xor_col38 = eval.u16_as_m31(xor_tmp_4da06_56);
    eval.set_col(38, xor_col38);
    eval.set_sub_input_word(6, split_16_low_part_size_8_output_tmp_4da06_49[0]);
    eval.set_sub_input_word(7, split_16_low_part_size_8_output_tmp_4da06_53[0]);
    eval.set_sub_input_word(8, xor_col38);
    eval.set_lookup_word(32, m31_112558620);
    eval.set_lookup_word(33, split_16_low_part_size_8_output_tmp_4da06_49[0]);
    eval.set_lookup_word(34, split_16_low_part_size_8_output_tmp_4da06_53[0]);
    eval.set_lookup_word(35, xor_col38);
    let wg_v109 = eval.u16_from_m31(ms_8_bits_col34);
    let wg_v110 = eval.u16_from_m31(ms_8_bits_col36);
    let xor_tmp_4da06_58 = eval.u16_xor(wg_v109, wg_v110);
    let xor_col39 = eval.u16_as_m31(xor_tmp_4da06_58);
    eval.set_col(39, xor_col39);
    eval.set_sub_input_word(9, ms_8_bits_col34);
    eval.set_sub_input_word(10, ms_8_bits_col36);
    eval.set_sub_input_word(11, xor_col39);
    eval.set_lookup_word(36, m31_112558620);
    eval.set_lookup_word(37, ms_8_bits_col34);
    eval.set_lookup_word(38, ms_8_bits_col36);
    eval.set_lookup_word(39, xor_col39);
    let wg_v111 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_51[0]);
    let wg_v112 = eval.u16_from_m31(split_16_low_part_size_8_output_tmp_4da06_55[0]);
    let xor_tmp_4da06_60 = eval.u16_xor(wg_v111, wg_v112);
    let xor_col40 = eval.u16_as_m31(xor_tmp_4da06_60);
    eval.set_col(40, xor_col40);
    eval.set_sub_input_word(18, split_16_low_part_size_8_output_tmp_4da06_51[0]);
    eval.set_sub_input_word(19, split_16_low_part_size_8_output_tmp_4da06_55[0]);
    eval.set_sub_input_word(20, xor_col40);
    eval.set_lookup_word(40, m31_521092554);
    eval.set_lookup_word(41, split_16_low_part_size_8_output_tmp_4da06_51[0]);
    eval.set_lookup_word(42, split_16_low_part_size_8_output_tmp_4da06_55[0]);
    eval.set_lookup_word(43, xor_col40);
    let wg_v113 = eval.u16_from_m31(ms_8_bits_col35);
    let wg_v114 = eval.u16_from_m31(ms_8_bits_col37);
    let xor_tmp_4da06_62 = eval.u16_xor(wg_v113, wg_v114);
    let xor_col41 = eval.u16_as_m31(xor_tmp_4da06_62);
    eval.set_col(41, xor_col41);
    eval.set_sub_input_word(21, ms_8_bits_col35);
    eval.set_sub_input_word(22, ms_8_bits_col37);
    eval.set_sub_input_word(23, xor_col41);
    eval.set_lookup_word(44, m31_521092554);
    eval.set_lookup_word(45, ms_8_bits_col35);
    eval.set_lookup_word(46, ms_8_bits_col37);
    eval.set_lookup_word(47, xor_col41);
    let wg_v115 = eval.m31_mul(xor_col40, m31_256);
    let wg_v116 = eval.m31_add(xor_col39, wg_v115);
    let wg_v117 = eval.m31_mul(xor_col38, m31_256);
    let wg_v118 = eval.m31_add(xor_col41, wg_v117);
    let xor_rot_8_output_tmp_4da06_64 = eval.u32_from_limbs(wg_v116, wg_v118);
    let xor_rot_32_r_8_output_tmp_4da06_65 = xor_rot_8_output_tmp_4da06_64;
    let wg_v119 = eval.u32_add(
        triple_sum_32_output_tmp_4da06_25,
        xor_rot_32_r_8_output_tmp_4da06_65,
    );
    let wg_v120 = eval.u32_const(0);
    let triple_sum32_res_tmp_4da06_66 = eval.u32_add(wg_v119, wg_v120);
    let wg_v121 = eval.u32_low(triple_sum32_res_tmp_4da06_66);
    let triple_sum32_res_limb_0_col42 = eval.u16_as_m31(wg_v121);
    eval.set_col(42, triple_sum32_res_limb_0_col42);
    let wg_v122 = eval.u32_high(triple_sum32_res_tmp_4da06_66);
    let triple_sum32_res_limb_1_col43 = eval.u16_as_m31(wg_v122);
    eval.set_col(43, triple_sum32_res_limb_1_col43);
    let triple_sum_32_output_tmp_4da06_69 = triple_sum32_res_tmp_4da06_66;
    let wg_v123 = eval.u32_low(xor_rot_32_r_12_output_tmp_4da06_43);
    let ms_9_bits_tmp_4da06_70 = eval.u16_shr(wg_v123, 7);
    let ms_9_bits_col44 = eval.u16_as_m31(ms_9_bits_tmp_4da06_70);
    eval.set_col(44, ms_9_bits_col44);
    let wg_v124 = eval.u32_low(xor_rot_12_output_tmp_4da06_42);
    let wg_v125 = eval.u16_as_m31(wg_v124);
    let wg_v126 = eval.m31_mul(ms_9_bits_col44, m31_128);
    let wg_v127 = eval.m31_sub(wg_v125, wg_v126);
    let split_16_low_part_size_7_output_tmp_4da06_71 = [wg_v127, ms_9_bits_col44];
    let wg_v128 = eval.u32_high(xor_rot_32_r_12_output_tmp_4da06_43);
    let ms_9_bits_tmp_4da06_72 = eval.u16_shr(wg_v128, 7);
    let ms_9_bits_col45 = eval.u16_as_m31(ms_9_bits_tmp_4da06_72);
    eval.set_col(45, ms_9_bits_col45);
    let wg_v129 = eval.u32_high(xor_rot_12_output_tmp_4da06_42);
    let wg_v130 = eval.u16_as_m31(wg_v129);
    let wg_v131 = eval.m31_mul(ms_9_bits_col45, m31_128);
    let wg_v132 = eval.m31_sub(wg_v130, wg_v131);
    let split_16_low_part_size_7_output_tmp_4da06_73 = [wg_v132, ms_9_bits_col45];
    let wg_v133 = eval.u32_low(triple_sum_32_output_tmp_4da06_69);
    let ms_9_bits_tmp_4da06_74 = eval.u16_shr(wg_v133, 7);
    let ms_9_bits_col46 = eval.u16_as_m31(ms_9_bits_tmp_4da06_74);
    eval.set_col(46, ms_9_bits_col46);
    let wg_v134 = eval.m31_mul(ms_9_bits_col46, m31_128);
    let wg_v135 = eval.m31_sub(triple_sum32_res_limb_0_col42, wg_v134);
    let split_16_low_part_size_7_output_tmp_4da06_75 = [wg_v135, ms_9_bits_col46];
    let wg_v136 = eval.u32_high(triple_sum_32_output_tmp_4da06_69);
    let ms_9_bits_tmp_4da06_76 = eval.u16_shr(wg_v136, 7);
    let ms_9_bits_col47 = eval.u16_as_m31(ms_9_bits_tmp_4da06_76);
    eval.set_col(47, ms_9_bits_col47);
    let wg_v137 = eval.m31_mul(ms_9_bits_col47, m31_128);
    let wg_v138 = eval.m31_sub(triple_sum32_res_limb_1_col43, wg_v137);
    let split_16_low_part_size_7_output_tmp_4da06_77 = [wg_v138, ms_9_bits_col47];
    let wg_v139 = eval.u16_from_m31(split_16_low_part_size_7_output_tmp_4da06_71[0]);
    let wg_v140 = eval.u16_from_m31(split_16_low_part_size_7_output_tmp_4da06_75[0]);
    let xor_tmp_4da06_78 = eval.u16_xor(wg_v139, wg_v140);
    let xor_col48 = eval.u16_as_m31(xor_tmp_4da06_78);
    eval.set_col(48, xor_col48);
    eval.set_sub_input_word(36, split_16_low_part_size_7_output_tmp_4da06_71[0]);
    eval.set_sub_input_word(37, split_16_low_part_size_7_output_tmp_4da06_75[0]);
    eval.set_sub_input_word(38, xor_col48);
    eval.set_lookup_word(48, m31_62225763);
    eval.set_lookup_word(49, split_16_low_part_size_7_output_tmp_4da06_71[0]);
    eval.set_lookup_word(50, split_16_low_part_size_7_output_tmp_4da06_75[0]);
    eval.set_lookup_word(51, xor_col48);
    let wg_v141 = eval.u16_from_m31(ms_9_bits_col44);
    let wg_v142 = eval.u16_from_m31(ms_9_bits_col46);
    let xor_tmp_4da06_80 = eval.u16_xor(wg_v141, wg_v142);
    let xor_col49 = eval.u16_as_m31(xor_tmp_4da06_80);
    eval.set_col(49, xor_col49);
    eval.set_sub_input_word(42, ms_9_bits_col44);
    eval.set_sub_input_word(43, ms_9_bits_col46);
    eval.set_sub_input_word(44, xor_col49);
    eval.set_lookup_word(52, m31_95781001);
    eval.set_lookup_word(53, ms_9_bits_col44);
    eval.set_lookup_word(54, ms_9_bits_col46);
    eval.set_lookup_word(55, xor_col49);
    let wg_v143 = eval.u16_from_m31(split_16_low_part_size_7_output_tmp_4da06_73[0]);
    let wg_v144 = eval.u16_from_m31(split_16_low_part_size_7_output_tmp_4da06_77[0]);
    let xor_tmp_4da06_82 = eval.u16_xor(wg_v143, wg_v144);
    let xor_col50 = eval.u16_as_m31(xor_tmp_4da06_82);
    eval.set_col(50, xor_col50);
    eval.set_sub_input_word(39, split_16_low_part_size_7_output_tmp_4da06_73[0]);
    eval.set_sub_input_word(40, split_16_low_part_size_7_output_tmp_4da06_77[0]);
    eval.set_sub_input_word(41, xor_col50);
    eval.set_lookup_word(56, m31_62225763);
    eval.set_lookup_word(57, split_16_low_part_size_7_output_tmp_4da06_73[0]);
    eval.set_lookup_word(58, split_16_low_part_size_7_output_tmp_4da06_77[0]);
    eval.set_lookup_word(59, xor_col50);
    let wg_v145 = eval.u16_from_m31(ms_9_bits_col45);
    let wg_v146 = eval.u16_from_m31(ms_9_bits_col47);
    let xor_tmp_4da06_84 = eval.u16_xor(wg_v145, wg_v146);
    let xor_col51 = eval.u16_as_m31(xor_tmp_4da06_84);
    eval.set_col(51, xor_col51);
    eval.set_sub_input_word(45, ms_9_bits_col45);
    eval.set_sub_input_word(46, ms_9_bits_col47);
    eval.set_sub_input_word(47, xor_col51);
    eval.set_lookup_word(60, m31_95781001);
    eval.set_lookup_word(61, ms_9_bits_col45);
    eval.set_lookup_word(62, ms_9_bits_col47);
    eval.set_lookup_word(63, xor_col51);
    let wg_v147 = eval.m31_mul(xor_col50, m31_512);
    let wg_v148 = eval.m31_add(xor_col49, wg_v147);
    let wg_v149 = eval.m31_mul(xor_col48, m31_512);
    let wg_v150 = eval.m31_add(xor_col51, wg_v149);
    let xor_rot_7_output_tmp_4da06_86 = eval.u32_from_limbs(wg_v148, wg_v150);
    let xor_rot_32_r_7_output_tmp_4da06_87 = xor_rot_7_output_tmp_4da06_86;
    let enabler_col52 = eval.enabler();
    eval.set_col(52, enabler_col52);
    eval.set_lookup_word(64, m31_1139985212);
    eval.set_lookup_word(65, input_limb_0_col0);
    eval.set_lookup_word(66, input_limb_1_col1);
    eval.set_lookup_word(67, input_limb_2_col2);
    eval.set_lookup_word(68, input_limb_3_col3);
    eval.set_lookup_word(69, input_limb_4_col4);
    eval.set_lookup_word(70, input_limb_5_col5);
    eval.set_lookup_word(71, input_limb_6_col6);
    eval.set_lookup_word(72, input_limb_7_col7);
    eval.set_lookup_word(73, input_limb_8_col8);
    eval.set_lookup_word(74, input_limb_9_col9);
    eval.set_lookup_word(75, input_limb_10_col10);
    eval.set_lookup_word(76, input_limb_11_col11);
    eval.set_lookup_word(77, triple_sum32_res_limb_0_col32);
    eval.set_lookup_word(78, triple_sum32_res_limb_1_col33);
    let wg_v151 = eval.u32_low(xor_rot_7_output_tmp_4da06_86);
    let wg_v152 = eval.u16_as_m31(wg_v151);
    eval.set_lookup_word(79, wg_v152);
    let wg_v153 = eval.u32_high(xor_rot_7_output_tmp_4da06_86);
    let wg_v154 = eval.u16_as_m31(wg_v153);
    eval.set_lookup_word(80, wg_v154);
    eval.set_lookup_word(81, triple_sum32_res_limb_0_col42);
    eval.set_lookup_word(82, triple_sum32_res_limb_1_col43);
    let wg_v155 = eval.u32_low(xor_rot_8_output_tmp_4da06_64);
    let wg_v156 = eval.u16_as_m31(wg_v155);
    eval.set_lookup_word(83, wg_v156);
    let wg_v157 = eval.u32_high(xor_rot_8_output_tmp_4da06_64);
    let wg_v158 = eval.u16_as_m31(wg_v157);
    eval.set_lookup_word(84, wg_v158);
    eval.set_lookup_word(85, m31_1);
    eval.set_lookup_word(86, enabler_col52);
}

/// Generic SIMD driver: same allocation as `write_trace_simd`, but each row runs
/// `blake_g_row_body` on a per-row `SimdWitnessEval`, then reconstructs the concrete
/// `LookupData` / `SubComponentInputs` from the eval's flat scratch. Module-private (it
/// returns the module-private `LookupData` / `SubComponentInputs`; wider visibility would
/// be E0446 and force a change OUTSIDE this block). External callers use the `pub(crate)`
/// `write_trace_generic` method or the `#[cfg(test)]` `generic_simd_diff` harness.
#[allow(clippy::type_complexity)]
#[allow(unused_variables)]
#[allow(dead_code)]
fn write_trace_generic_simd(
    inputs: Vec<PackedInputType>,
    n_rows: usize,
    verify_bitwise_xor_8_state: &verify_bitwise_xor_8::ClaimGenerator,
    verify_bitwise_xor_12_state: &verify_bitwise_xor_12::ClaimGenerator,
    verify_bitwise_xor_4_state: &verify_bitwise_xor_4::ClaimGenerator,
    verify_bitwise_xor_7_state: &verify_bitwise_xor_7::ClaimGenerator,
    verify_bitwise_xor_9_state: &verify_bitwise_xor_9::ClaimGenerator,
) -> (
    ComponentTrace<N_TRACE_COLUMNS>,
    LookupData,
    SubComponentInputs,
) {
    let log_n_packed_rows = inputs.len().ilog2();
    let log_size = log_n_packed_rows + LOG_N_LANES;
    let (mut trace, mut lookup_data, mut sub_component_inputs) = unsafe {
        (
            ComponentTrace::<N_TRACE_COLUMNS>::uninitialized(log_size),
            LookupData::uninitialized(log_n_packed_rows),
            SubComponentInputs::uninitialized(log_n_packed_rows),
        )
    };
    let enabler_col = Enabler::new(n_rows);
    (
        trace.par_iter_mut(),
        lookup_data.par_iter_mut(),
        sub_component_inputs.par_iter_mut(),
        inputs.into_par_iter(),
    )
        .into_par_iter()
        .enumerate()
        .for_each(
            |(row_index, (row, lookup_data, sub_component_inputs, blake_g_input))| {
                let mut eval = SimdWitnessEval::new(
                    row,
                    None,
                    None,
                    vec![
                        blake_g_input[0].simd,
                        blake_g_input[1].simd,
                        blake_g_input[2].simd,
                        blake_g_input[3].simd,
                        blake_g_input[4].simd,
                        blake_g_input[5].simd,
                    ],
                    row_index,
                    &enabler_col,
                    N_LOOKUP_WORDS,
                    N_SUB_INPUT_WORDS,
                );
                blake_g_row_body(&mut eval);
                let lw = eval.lookup_scratch();
                *lookup_data.verify_bitwise_xor_8_0 = [lw[0], lw[1], lw[2], lw[3]];
                *lookup_data.verify_bitwise_xor_8_1 = [lw[4], lw[5], lw[6], lw[7]];
                *lookup_data.verify_bitwise_xor_8_b_2 = [lw[8], lw[9], lw[10], lw[11]];
                *lookup_data.verify_bitwise_xor_8_b_3 = [lw[12], lw[13], lw[14], lw[15]];
                *lookup_data.verify_bitwise_xor_12_4 = [lw[16], lw[17], lw[18], lw[19]];
                *lookup_data.verify_bitwise_xor_4_5 = [lw[20], lw[21], lw[22], lw[23]];
                *lookup_data.verify_bitwise_xor_12_6 = [lw[24], lw[25], lw[26], lw[27]];
                *lookup_data.verify_bitwise_xor_4_7 = [lw[28], lw[29], lw[30], lw[31]];
                *lookup_data.verify_bitwise_xor_8_8 = [lw[32], lw[33], lw[34], lw[35]];
                *lookup_data.verify_bitwise_xor_8_9 = [lw[36], lw[37], lw[38], lw[39]];
                *lookup_data.verify_bitwise_xor_8_b_10 = [lw[40], lw[41], lw[42], lw[43]];
                *lookup_data.verify_bitwise_xor_8_b_11 = [lw[44], lw[45], lw[46], lw[47]];
                *lookup_data.verify_bitwise_xor_7_12 = [lw[48], lw[49], lw[50], lw[51]];
                *lookup_data.verify_bitwise_xor_9_13 = [lw[52], lw[53], lw[54], lw[55]];
                *lookup_data.verify_bitwise_xor_7_14 = [lw[56], lw[57], lw[58], lw[59]];
                *lookup_data.verify_bitwise_xor_9_15 = [lw[60], lw[61], lw[62], lw[63]];
                *lookup_data.blake_g_16 = [
                    lw[64], lw[65], lw[66], lw[67], lw[68], lw[69], lw[70], lw[71], lw[72], lw[73],
                    lw[74], lw[75], lw[76], lw[77], lw[78], lw[79], lw[80], lw[81], lw[82], lw[83],
                    lw[84],
                ];
                *lookup_data.mults_0 = lw[85];
                *lookup_data.mults_1 = lw[86];
                let sw = eval.sub_scratch();
                *sub_component_inputs.verify_bitwise_xor_8[0] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[0]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[1]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[2]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_8[1] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[3]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[4]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[5]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_8[2] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[6]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[7]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[8]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_8[3] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[9]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[10]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[11]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_8_b[0] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[12]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[13]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[14]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_8_b[1] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[15]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[16]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[17]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_8_b[2] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[18]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[19]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[20]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_8_b[3] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[21]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[22]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[23]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_12[0] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[24]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[25]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[26]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_12[1] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[27]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[28]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[29]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_4[0] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[30]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[31]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[32]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_4[1] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[33]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[34]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[35]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_7[0] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[36]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[37]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[38]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_7[1] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[39]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[40]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[41]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_9[0] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[42]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[43]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[44]) },
                ];
                *sub_component_inputs.verify_bitwise_xor_9[1] = [
                    unsafe { PackedM31::from_simd_unchecked(sw[45]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[46]) },
                    unsafe { PackedM31::from_simd_unchecked(sw[47]) },
                ];
            },
        );
    (trace, lookup_data, sub_component_inputs)
}

impl ClaimGenerator {
    /// Generic-path counterpart of [`ClaimGenerator::write_trace`]: identical shape, but
    /// the base trace is produced by `write_trace_generic_simd`.
    #[allow(dead_code)]
    pub(crate) fn write_trace_generic(
        self,
        verify_bitwise_xor_8_state: &verify_bitwise_xor_8::ClaimGenerator,
        verify_bitwise_xor_12_state: &verify_bitwise_xor_12::ClaimGenerator,
        verify_bitwise_xor_4_state: &verify_bitwise_xor_4::ClaimGenerator,
        verify_bitwise_xor_7_state: &verify_bitwise_xor_7::ClaimGenerator,
        verify_bitwise_xor_9_state: &verify_bitwise_xor_9::ClaimGenerator,
    ) -> (
        ComponentTrace<N_TRACE_COLUMNS>,
        Claim,
        InteractionClaimGenerator,
    ) {
        let mut packed_inputs = self.packed_inputs.into_inner().unwrap();
        assert!(!packed_inputs.is_empty());
        assert!(self.remainder_inputs.lock().unwrap().is_empty());
        let n_vec_rows = packed_inputs.len();
        let n_rows = n_vec_rows * N_LANES;
        let packed_size = n_vec_rows.next_power_of_two();
        let log_size = packed_size.ilog2() + LOG_N_LANES;
        packed_inputs.resize(packed_size, *packed_inputs.first().unwrap());
        let (trace, lookup_data, sub_component_inputs) = write_trace_generic_simd(
            packed_inputs,
            n_rows,
            verify_bitwise_xor_8_state,
            verify_bitwise_xor_12_state,
            verify_bitwise_xor_4_state,
            verify_bitwise_xor_7_state,
            verify_bitwise_xor_9_state,
        );
        for inputs in sub_component_inputs.verify_bitwise_xor_8 {
            add_inputs(
                verify_bitwise_xor_8_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_8_b {
            add_inputs(
                verify_bitwise_xor_8_state,
                &inputs,
                inputs.len() * N_LANES,
                1,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_12 {
            add_inputs(
                verify_bitwise_xor_12_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_4 {
            add_inputs(
                verify_bitwise_xor_4_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_7 {
            add_inputs(
                verify_bitwise_xor_7_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        for inputs in sub_component_inputs.verify_bitwise_xor_9 {
            add_inputs(
                verify_bitwise_xor_9_state,
                &inputs,
                inputs.len() * N_LANES,
                0,
            );
        }
        (
            trace,
            Claim { log_size },
            InteractionClaimGenerator {
                log_size,
                lookup_data,
            },
        )
    }
}

/// Record the `blake_g` per-row body into witness-JIT bytecode
/// (statement-independent — recorded once). EXTENDED ops (if any) surface in
/// `RecordingOutput::poison_ops` — the honest ISA-V2 census, not a failure.
#[allow(dead_code)]
pub(crate) fn record_blake_g() -> RecordingOutput {
    let mut eval = RecordingWitnessEval::with_slots("blake_g", 6, Some(7));
    blake_g_row_body(&mut eval);
    eval.finish()
}

crate::jit_lookup_accessor! {
    87;
    verify_bitwise_xor_8_0: 4,
    verify_bitwise_xor_8_1: 4,
    verify_bitwise_xor_8_b_2: 4,
    verify_bitwise_xor_8_b_3: 4,
    verify_bitwise_xor_12_4: 4,
    verify_bitwise_xor_4_5: 4,
    verify_bitwise_xor_12_6: 4,
    verify_bitwise_xor_4_7: 4,
    verify_bitwise_xor_8_8: 4,
    verify_bitwise_xor_8_9: 4,
    verify_bitwise_xor_8_b_10: 4,
    verify_bitwise_xor_8_b_11: 4,
    verify_bitwise_xor_7_12: 4,
    verify_bitwise_xor_9_13: 4,
    verify_bitwise_xor_7_14: 4,
    verify_bitwise_xor_9_15: 4,
    blake_g_16: 21,
    mults_0: scalar,
    mults_1: scalar,
}

/// Device-DAG feed layout (facts, DECLARATION order): one entry per
/// `SubComponentInputs` instance — (field, instance, downstream state
/// param, relation_index, flat word base, words per instance).
#[allow(dead_code)]
pub(crate) const SUB_FEED_LAYOUT: &[(&str, usize, &str, u32, usize, usize)] = &[
    (
        "verify_bitwise_xor_8",
        0,
        "verify_bitwise_xor_8_state",
        0,
        0,
        3,
    ),
    (
        "verify_bitwise_xor_8",
        1,
        "verify_bitwise_xor_8_state",
        0,
        3,
        3,
    ),
    (
        "verify_bitwise_xor_8",
        2,
        "verify_bitwise_xor_8_state",
        0,
        6,
        3,
    ),
    (
        "verify_bitwise_xor_8",
        3,
        "verify_bitwise_xor_8_state",
        0,
        9,
        3,
    ),
    (
        "verify_bitwise_xor_8_b",
        0,
        "verify_bitwise_xor_8_state",
        1,
        12,
        3,
    ),
    (
        "verify_bitwise_xor_8_b",
        1,
        "verify_bitwise_xor_8_state",
        1,
        15,
        3,
    ),
    (
        "verify_bitwise_xor_8_b",
        2,
        "verify_bitwise_xor_8_state",
        1,
        18,
        3,
    ),
    (
        "verify_bitwise_xor_8_b",
        3,
        "verify_bitwise_xor_8_state",
        1,
        21,
        3,
    ),
    (
        "verify_bitwise_xor_12",
        0,
        "verify_bitwise_xor_12_state",
        0,
        24,
        3,
    ),
    (
        "verify_bitwise_xor_12",
        1,
        "verify_bitwise_xor_12_state",
        0,
        27,
        3,
    ),
    (
        "verify_bitwise_xor_4",
        0,
        "verify_bitwise_xor_4_state",
        0,
        30,
        3,
    ),
    (
        "verify_bitwise_xor_4",
        1,
        "verify_bitwise_xor_4_state",
        0,
        33,
        3,
    ),
    (
        "verify_bitwise_xor_7",
        0,
        "verify_bitwise_xor_7_state",
        0,
        36,
        3,
    ),
    (
        "verify_bitwise_xor_7",
        1,
        "verify_bitwise_xor_7_state",
        0,
        39,
        3,
    ),
    (
        "verify_bitwise_xor_9",
        0,
        "verify_bitwise_xor_9_state",
        0,
        42,
        3,
    ),
    (
        "verify_bitwise_xor_9",
        1,
        "verify_bitwise_xor_9_state",
        0,
        45,
        3,
    ),
];

/// §6a device-interaction descriptors (facts, COLUMN order): one entry
/// per logup column — (a_field, a_mult, a_neg, b_field, b_mult, b_neg);
/// b_field == "" for a trailing solo column. mult encoding: "1" = one,
/// "enabler" = the real-row enabler, else a scalar lookup-data field.
#[allow(dead_code)]
pub(crate) const JIT_LOGUP_DESCS: &[(&str, &str, bool, &str, &str, bool)] = &[
    (
        "verify_bitwise_xor_8_0",
        "mults_0",
        false,
        "verify_bitwise_xor_8_1",
        "mults_0",
        false,
    ),
    (
        "verify_bitwise_xor_8_b_2",
        "mults_0",
        false,
        "verify_bitwise_xor_8_b_3",
        "mults_0",
        false,
    ),
    (
        "verify_bitwise_xor_12_4",
        "mults_0",
        false,
        "verify_bitwise_xor_4_5",
        "mults_0",
        false,
    ),
    (
        "verify_bitwise_xor_12_6",
        "mults_0",
        false,
        "verify_bitwise_xor_4_7",
        "mults_0",
        false,
    ),
    (
        "verify_bitwise_xor_8_8",
        "mults_0",
        false,
        "verify_bitwise_xor_8_9",
        "mults_0",
        false,
    ),
    (
        "verify_bitwise_xor_8_b_10",
        "mults_0",
        false,
        "verify_bitwise_xor_8_b_11",
        "mults_0",
        false,
    ),
    (
        "verify_bitwise_xor_7_12",
        "mults_0",
        false,
        "verify_bitwise_xor_9_13",
        "mults_0",
        false,
    ),
    (
        "verify_bitwise_xor_7_14",
        "mults_0",
        false,
        "verify_bitwise_xor_9_15",
        "mults_0",
        false,
    ),
    ("blake_g_16", "mults_1", true, "", "", false),
];

// ---- Test-only surface for the byte-equality gate ---------------------------------

fn lookup_data_flat(ld: &LookupData) -> Vec<Vec<PackedM31>> {
    vec![
        ld.verify_bitwise_xor_8_0
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_8_1
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_8_b_2
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_8_b_3
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_12_4
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_4_5
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_12_6
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_4_7
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_8_8
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_8_9
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_8_b_10
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_8_b_11
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_7_12
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_9_13
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_7_14
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.verify_bitwise_xor_9_15
            .iter()
            .flatten()
            .copied()
            .collect(),
        ld.blake_g_16.iter().flatten().copied().collect(),
        ld.mults_0.clone(),
        ld.mults_1.clone(),
    ]
}

#[cfg(test)]
pub(crate) fn test_lookup_data_flat(ig: &InteractionClaimGenerator) -> Vec<Vec<PackedM31>> {
    lookup_data_flat(&ig.lookup_data)
}

fn sub_inputs_flat(sci: &SubComponentInputs) -> Vec<Vec<Simd<u32, N_LANES>>> {
    vec![
        sci.verify_bitwise_xor_8[0]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_8[1]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_8[2]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_8[3]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_8_b[0]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_8_b[1]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_8_b[2]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_8_b[3]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_12[0]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_12[1]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_4[0]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_4[1]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_7[0]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_7[1]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_9[0]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
        sci.verify_bitwise_xor_9[1]
            .iter()
            .flat_map(|t| vec![t[0].into_simd(), t[1].into_simd(), t[2].into_simd()])
            .collect::<Vec<_>>(),
    ]
}

/// Byte-comparison bundle (only public types cross the module boundary).
#[cfg(test)]
pub(crate) struct GenericSimdDiff {
    pub log_size: u32,
    pub orig_rows: Vec<[M31; N_TRACE_COLUMNS]>,
    pub gen_rows: Vec<[M31; N_TRACE_COLUMNS]>,
    pub orig_lookup: Vec<Vec<PackedM31>>,
    pub gen_lookup: Vec<Vec<PackedM31>>,
    pub orig_sub: Vec<Vec<Simd<u32, N_LANES>>>,
    pub gen_sub: Vec<Vec<Simd<u32, N_LANES>>>,
    pub orig_interaction_cols: Vec<Vec<M31>>,
    pub gen_interaction_cols: Vec<Vec<M31>>,
    pub orig_claimed_sum: SecureField,
    pub gen_claimed_sum: SecureField,
}

/// Run BOTH SIMD writers on the same (pure-read) states and return public compare data.
#[cfg(test)]
pub(crate) fn generic_simd_diff(
    inputs: Vec<PackedInputType>,
    n_rows: usize,
    verify_bitwise_xor_8_state: &verify_bitwise_xor_8::ClaimGenerator,
    verify_bitwise_xor_12_state: &verify_bitwise_xor_12::ClaimGenerator,
    verify_bitwise_xor_4_state: &verify_bitwise_xor_4::ClaimGenerator,
    verify_bitwise_xor_7_state: &verify_bitwise_xor_7::ClaimGenerator,
    verify_bitwise_xor_9_state: &verify_bitwise_xor_9::ClaimGenerator,
) -> GenericSimdDiff {
    let (trace_o, ld_o, sci_o) = write_trace_simd(
        inputs.clone(),
        n_rows.clone(),
        verify_bitwise_xor_8_state,
        verify_bitwise_xor_12_state,
        verify_bitwise_xor_4_state,
        verify_bitwise_xor_7_state,
        verify_bitwise_xor_9_state,
    );
    let (trace_g, ld_g, sci_g) = write_trace_generic_simd(
        inputs,
        n_rows,
        verify_bitwise_xor_8_state,
        verify_bitwise_xor_12_state,
        verify_bitwise_xor_4_state,
        verify_bitwise_xor_7_state,
        verify_bitwise_xor_9_state,
    );
    let log_size = trace_o.log_size();
    let orig_rows = (0..(1usize << log_size))
        .map(|r| trace_o.row_at(r))
        .collect();
    let gen_rows = (0..(1usize << log_size))
        .map(|r| trace_g.row_at(r))
        .collect();
    let orig_lookup = lookup_data_flat(&ld_o);
    let gen_lookup = lookup_data_flat(&ld_g);
    let orig_sub = sub_inputs_flat(&sci_o);
    let gen_sub = sub_inputs_flat(&sci_g);
    let common = relations::CommonLookupElements::dummy();
    let (raw_o, _) = InteractionClaimGenerator {
        log_size,
        lookup_data: ld_o,
    }
    .write_interaction_trace(&common);
    let (raw_g, _) = InteractionClaimGenerator {
        log_size,
        lookup_data: ld_g,
    }
    .write_interaction_trace(&common);
    let (cols_o, orig_claimed_sum) = raw_o.finalize_on_simd();
    let (cols_g, gen_claimed_sum) = raw_g.finalize_on_simd();
    let orig_interaction_cols = cols_o.iter().map(|c| c.values.to_cpu()).collect();
    let gen_interaction_cols = cols_g.iter().map(|c| c.values.to_cpu()).collect();
    GenericSimdDiff {
        log_size,
        orig_rows,
        gen_rows,
        orig_lookup,
        gen_lookup,
        orig_sub,
        gen_sub,
        orig_interaction_cols,
        gen_interaction_cols,
        orig_claimed_sum,
        gen_claimed_sum,
    }
}
// === END witness_genericize ===

#[derive(Uninitialized, IterMut, ParIterMut)]
struct LookupData {
    verify_bitwise_xor_8_0: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_8_1: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_8_b_2: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_8_b_3: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_12_4: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_4_5: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_12_6: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_4_7: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_8_8: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_8_9: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_8_b_10: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_8_b_11: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_7_12: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_9_13: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_7_14: Vec<[PackedM31; 4]>,
    verify_bitwise_xor_9_15: Vec<[PackedM31; 4]>,
    blake_g_16: Vec<[PackedM31; 21]>,
    mults_0: Vec<PackedM31>,
    mults_1: Vec<PackedM31>,
}

pub struct InteractionClaimGenerator {
    log_size: u32,
    lookup_data: LookupData,
}
impl InteractionClaimGenerator {
    pub fn write_interaction_trace(
        self,
        common_lookup_elements: &relations::CommonLookupElements,
    ) -> (RawLogupTrace, impl FnOnce(SecureField) -> InteractionClaim) {
        let mut logup_gen = unsafe { RawLogupTraceGenerator::uninitialized(self.log_size) };

        // Sum logup terms in pairs.
        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_8_0,
            &self.lookup_data.verify_bitwise_xor_8_1,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_8_b_2,
            &self.lookup_data.verify_bitwise_xor_8_b_3,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_12_4,
            &self.lookup_data.verify_bitwise_xor_4_5,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_12_6,
            &self.lookup_data.verify_bitwise_xor_4_7,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_8_8,
            &self.lookup_data.verify_bitwise_xor_8_9,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_8_b_10,
            &self.lookup_data.verify_bitwise_xor_8_b_11,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_7_12,
            &self.lookup_data.verify_bitwise_xor_9_13,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.verify_bitwise_xor_7_14,
            &self.lookup_data.verify_bitwise_xor_9_15,
            &self.lookup_data.mults_0,
            &self.lookup_data.mults_0,
        )
            .into_par_iter()
            .for_each(|(writer, values0, values1, mult0, mult1)| {
                let denom0: PackedQM31 = common_lookup_elements.combine(values0);
                let denom1: PackedQM31 = common_lookup_elements.combine(values1);
                writer.write_frac(denom0 * *mult1 + denom1 * *mult0, denom0 * denom1);
            });
        col_gen.finalize_col();

        // Sum last logup term.
        let mut col_gen = logup_gen.new_col();
        (
            col_gen.par_iter_mut(),
            &self.lookup_data.blake_g_16,
            self.lookup_data.mults_1,
        )
            .into_par_iter()
            .for_each(|(writer, values, mult)| {
                let denom = common_lookup_elements.combine(values);
                writer.write_frac((-mult).into(), denom);
            });
        col_gen.finalize_col();

        (logup_gen.into_raw(), |claimed_sum| InteractionClaim {
            claimed_sum,
        })
    }
}
