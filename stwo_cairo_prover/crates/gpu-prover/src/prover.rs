    ...
    CudaBackend: CairoBackend<MC>,
{
    ...
    /// A/B measurements. The normal constructor remains adaptive and retains
    /// group-direct only with a sealed run-sum receipt; this path accepts no
...
mod resident_transcript_mirror_tests {
    ...
    #[test]
    fn packed_numerator_measurement_policy_accepts_only_loaded_adaptive_tuple() {
        let adaptive = ProtocolPlanPolicy::replacement_v1(0x1234, 2048);
        let packed = packed_numerator_measurement_policy(adaptive).unwrap();
        assert_eq!(
            packed,
