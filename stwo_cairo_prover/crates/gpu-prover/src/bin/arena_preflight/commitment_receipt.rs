...
use stwo_backend_cuda::{
    CommitProgramTraffic, DomainCooperativeOperation, ProgressiveCommitStorageMode,
};
...
pub(crate) fn validate_dynamic_commitment_leaf_programs(
    ...
    for commitment in commitments {
        ...
        let expected_program = selected_backend == ResidentBackend::ReplacementV1 && dynamic;
        if commitment.domain_cooperative_program.is_some() != expected_program {
            return Err(format!(
                "{} domain-cooperative program presence drifted",
...
fn validate_replacement_commitment(
    ...
    let base = commitment.commit_program.as_ref().ok_or_else(|| {
        format!(
            "{} Mode-A commitment is missing its base program",
            tree_name(commitment.id)
        )
    })?;
    commitment
        ...
        .as_ref()
        .expect("presence checked before replacement admission")
        .validate_against(base)
        .map_err(|error| {
            format!(
                "{} Mode-A program/base identity mismatch: {error:?}",
                tree_name(commitment.id)
            )
        })
}
...
fn commitment_receipt(commitment: &PlannedCommitment) -> serde_json::Value {
    ...
    let program = commitment.domain_cooperative_program.as_ref();
    let exact_base_validation = program
        .zip(base)
    ...
    serde_json::json!({
        ...
        "domain_cooperative_program_present": program.is_some(),
        "domain_cooperative_program": program.map(|program| {
